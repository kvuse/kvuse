import { inject as Dt, getCurrentInstance as Re, onUnmounted as on, computed as O, ref as E, reactive as De, onMounted as Ee, nextTick as te, onActivated as nt, onDeactivated as Qe, isRef as $n, watch as Z, unref as oe, provide as ot, isVNode as Go, defineComponent as Q, createVNode as f, onBeforeUnmount as zt, watchEffect as _n, mergeProps as fe, Transition as eo, withDirectives as an, vShow as ln, Teleport as Qo, Fragment as Ne, onBeforeUpdate as Lo, createTextVNode as rn, createApp as qo, onUpdated as Fo, useCssVars as to, resolveComponent as St, openBlock as F, createBlock as Se, createSlots as Vo, withCtx as Pe, renderSlot as ke, createElementBlock as re, createElementVNode as me, toDisplayString as pe, normalizeStyle as Nt, renderList as $e, normalizeClass as cn, createCommentVNode as et, withModifiers as Wo, getCurrentScope as Ko, onScopeDispose as $o, readonly as _o, useAttrs as sn } from "vue";
const ae = (e) => e != null, It = (e) => typeof e == "function", at = (e) => e !== null && typeof e == "object", no = (e) => at(e) && It(e.then) && It(e.catch), Lt = (e) => Object.prototype.toString.call(e) === "[object Date]" && !Number.isNaN(e.getTime()), oo = (e) => typeof e == "number" || /^\d+(\.\d+)?$/.test(e), ea = () => lt ? /ios|iphone|ipad|ipod/.test(navigator.userAgent.toLowerCase()) : !1;
function ta() {
}
const ce = Object.assign, lt = typeof window < "u";
function En(e, t) {
  const n = t.split(".");
  let o = e;
  return n.forEach((l) => {
    var a;
    o = at(o) && (a = o[l]) != null ? a : "";
  }), o;
}
function Ie(e, t, n) {
  return t.reduce((o, l) => ((!n || e[l] !== void 0) && (o[l] = e[l]), o), {});
}
const pt = (e, t) => JSON.stringify(e) === JSON.stringify(t), Sn = (e) => Array.isArray(e) ? e : [e], je = null, Y = [Number, String], q = {
  type: Boolean,
  default: !0
}, Oe = (e) => ({
  type: e,
  required: !0
}), tt = () => ({
  type: Array,
  default: () => []
}), ao = (e) => ({
  type: Number,
  default: e
}), le = (e) => ({
  type: Y,
  default: e
}), V = (e) => ({
  type: String,
  default: e
});
var Le = typeof window < "u";
function Be(e) {
  return Le ? requestAnimationFrame(e) : -1;
}
function lo(e) {
  Le && cancelAnimationFrame(e);
}
function Ke(e) {
  Be(() => Be(e));
}
var na = (e) => e === window, In = (e, t) => ({
  top: 0,
  left: 0,
  right: e,
  bottom: t,
  width: e,
  height: t
}), he = (e) => {
  const t = oe(e);
  if (na(t)) {
    const n = t.innerWidth, o = t.innerHeight;
    return In(n, o);
  }
  return t != null && t.getBoundingClientRect ? t.getBoundingClientRect() : In(0, 0);
};
function oa(e = !1) {
  const t = E(e);
  return [t, (o = !t.value) => {
    t.value = o;
  }];
}
function qe(e) {
  const t = Dt(e, null);
  if (t) {
    const n = Re(), { link: o, unlink: l, internalChildren: a } = t;
    o(n), on(() => l(n));
    const c = O(() => a.indexOf(n));
    return {
      parent: t,
      index: c
    };
  }
  return {
    parent: null,
    index: E(-1)
  };
}
function aa(e) {
  const t = [], n = (o) => {
    Array.isArray(o) && o.forEach((l) => {
      var a;
      Go(l) && (t.push(l), (a = l.component) != null && a.subTree && (t.push(l.component.subTree), n(l.component.subTree.children)), l.children && n(l.children));
    });
  };
  return n(e), t;
}
var xn = (e, t) => {
  const n = e.indexOf(t);
  return n === -1 ? e.findIndex(
    (o) => t.key !== void 0 && t.key !== null && o.type === t.type && o.key === t.key
  ) : n;
};
function la(e, t, n) {
  const o = aa(e.subTree.children);
  n.sort(
    (a, c) => xn(o, a.vnode) - xn(o, c.vnode)
  );
  const l = n.map((a) => a.proxy);
  t.sort((a, c) => {
    const r = l.indexOf(a), i = l.indexOf(c);
    return r - i;
  });
}
function it(e) {
  const t = De([]), n = De([]), o = Re();
  return {
    children: t,
    linkChildren: (a) => {
      ot(
        e,
        Object.assign(
          {
            link: (i) => {
              i.proxy && (n.push(i), t.push(i.proxy), la(o, t, n));
            },
            unlink: (i) => {
              const u = n.indexOf(i);
              t.splice(u, 1), n.splice(u, 1);
            },
            children: t,
            internalChildren: n
          },
          a
        )
      );
    }
  };
}
function rt(e) {
  let t;
  Ee(() => {
    e(), te(() => {
      t = !0;
    });
  }), nt(() => {
    t && e();
  });
}
function ze(e, t, n = {}) {
  if (!Le)
    return;
  const { target: o = window, passive: l = !1, capture: a = !1 } = n;
  let c = !1, r;
  const i = (g) => {
    if (c)
      return;
    const d = oe(g);
    d && !r && (d.addEventListener(e, t, {
      capture: a,
      passive: l
    }), r = !0);
  }, u = (g) => {
    if (c)
      return;
    const d = oe(g);
    d && r && (d.removeEventListener(e, t, a), r = !1);
  };
  on(() => u(o)), Qe(() => u(o)), rt(() => i(o));
  let s;
  return $n(o) && (s = Z(o, (g, d) => {
    u(d), i(g);
  })), () => {
    s == null || s(), u(o), c = !0;
  };
}
var vt, Bt;
function ia() {
  if (!vt && (vt = E(0), Bt = E(0), Le)) {
    const e = () => {
      vt.value = window.innerWidth, Bt.value = window.innerHeight;
    };
    e(), window.addEventListener("resize", e, { passive: !0 }), window.addEventListener("orientationchange", e, { passive: !0 });
  }
  return { width: vt, height: Bt };
}
var ra = /scroll|auto|overlay/i, io = Le ? window : void 0;
function ca(e) {
  return e.tagName !== "HTML" && e.tagName !== "BODY" && e.nodeType === 1;
}
function ro(e, t = io) {
  let n = e;
  for (; n && n !== t && ca(n); ) {
    const { overflowY: o } = window.getComputedStyle(n);
    if (ra.test(o))
      return n;
    n = n.parentNode;
  }
  return t;
}
function wt(e, t = io) {
  const n = E();
  return Ee(() => {
    e.value && (n.value = ro(e.value, t));
  }), n;
}
var gt;
function sa() {
  if (!gt && (gt = E("visible"), Le)) {
    const e = () => {
      gt.value = document.hidden ? "hidden" : "visible";
    };
    e(), window.addEventListener("visibilitychange", e);
  }
  return gt;
}
var ua = Symbol("van-field");
function ct(e) {
  const t = "scrollTop" in e ? e.scrollTop : e.pageYOffset;
  return Math.max(t, 0);
}
function xt(e, t) {
  "scrollTop" in e ? e.scrollTop = t : e.scrollTo(e.scrollX, t);
}
function un() {
  return window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;
}
function dn(e) {
  xt(window, e), xt(document.body, e);
}
function Tn(e, t) {
  if (e === window)
    return 0;
  const n = t ? ct(t) : un();
  return he(e).top + n;
}
const da = ea();
function fa() {
  da && dn(un());
}
const va = (e) => e.stopPropagation();
function xe(e, t) {
  (typeof e.cancelable != "boolean" || e.cancelable) && e.preventDefault(), t && va(e);
}
function Ge(e) {
  const t = oe(e);
  if (!t)
    return !1;
  const n = window.getComputedStyle(t), o = n.display === "none", l = t.offsetParent === null && n.position !== "fixed";
  return o || l;
}
const { width: st, height: Ct } = ia();
function ge(e) {
  if (ae(e))
    return oo(e) ? `${e}px` : String(e);
}
function ga(e) {
  if (ae(e)) {
    if (Array.isArray(e))
      return {
        width: ge(e[0]),
        height: ge(e[1])
      };
    const t = ge(e);
    return {
      width: t,
      height: t
    };
  }
}
function co(e) {
  const t = {};
  return e !== void 0 && (t.zIndex = +e), t;
}
let Rt;
function ma() {
  if (!Rt) {
    const e = document.documentElement, t = e.style.fontSize || window.getComputedStyle(e).fontSize;
    Rt = parseFloat(t);
  }
  return Rt;
}
function Aa(e) {
  return e = e.replace(/rem/g, ""), +e * ma();
}
function ha(e) {
  return e = e.replace(/vw/g, ""), +e * st.value / 100;
}
function ya(e) {
  return e = e.replace(/vh/g, ""), +e * Ct.value / 100;
}
function fn(e) {
  if (typeof e == "number")
    return e;
  if (lt) {
    if (e.includes("rem"))
      return Aa(e);
    if (e.includes("vw"))
      return ha(e);
    if (e.includes("vh"))
      return ya(e);
  }
  return parseFloat(e);
}
const ba = /-(\w)/g, so = (e) => e.replace(ba, (t, n) => n.toUpperCase()), ka = (e) => e.replace(/([A-Z])/g, "-$1").toLowerCase().replace(/^-/, ""), Xe = (e, t, n) => Math.min(Math.max(e, t), n);
function Dn(e, t, n) {
  const o = e.indexOf(t);
  return o === -1 ? e : t === "-" && o !== 0 ? e.slice(0, o) : e.slice(0, o + 1) + e.slice(o).replace(n, "");
}
function Pa(e, t = !0, n = !0) {
  t ? e = Dn(e, ".", /\./g) : e = e.split(".")[0], n ? e = Dn(e, "-", /-/g) : e = e.replace(/-/, "");
  const o = t ? /[^-0-9.]/g : /[^-0-9]/g;
  return e.replace(o, "");
}
const { hasOwnProperty: Oa } = Object.prototype;
function Ea(e, t, n) {
  const o = t[n];
  ae(o) && (!Oa.call(e, n) || !at(o) ? e[n] = o : e[n] = uo(Object(e[n]), o));
}
function uo(e, t) {
  return Object.keys(t).forEach((n) => {
    Ea(e, t, n);
  }), e;
}
var Sa = {
  name: "姓名",
  tel: "电话",
  save: "保存",
  clear: "清空",
  cancel: "取消",
  confirm: "确认",
  delete: "删除",
  loading: "加载中...",
  noCoupon: "暂无优惠券",
  nameEmpty: "请填写姓名",
  addContact: "添加联系人",
  telInvalid: "请填写正确的电话",
  vanCalendar: {
    end: "结束",
    start: "开始",
    title: "日期选择",
    weekdays: ["日", "一", "二", "三", "四", "五", "六"],
    monthTitle: (e, t) => `${e}年${t}月`,
    rangePrompt: (e) => `最多选择 ${e} 天`
  },
  vanCascader: {
    select: "请选择"
  },
  vanPagination: {
    prev: "上一页",
    next: "下一页"
  },
  vanPullRefresh: {
    pulling: "下拉即可刷新...",
    loosing: "释放即可刷新..."
  },
  vanSubmitBar: {
    label: "合计:"
  },
  vanCoupon: {
    unlimited: "无门槛",
    discount: (e) => `${e}折`,
    condition: (e) => `满${e}元可用`
  },
  vanCouponCell: {
    title: "优惠券",
    count: (e) => `${e}张可用`
  },
  vanCouponList: {
    exchange: "兑换",
    close: "不使用",
    enable: "可用",
    disabled: "不可用",
    placeholder: "输入优惠码"
  },
  vanAddressEdit: {
    area: "地区",
    areaEmpty: "请选择地区",
    addressEmpty: "请填写详细地址",
    addressDetail: "详细地址",
    defaultAddress: "设为默认收货地址"
  },
  vanAddressList: {
    add: "新增地址"
  }
};
const zn = E("zh-CN"), wn = De({
  "zh-CN": Sa
}), Ia = {
  messages() {
    return wn[zn.value];
  },
  use(e, t) {
    zn.value = e, this.add({ [e]: t });
  },
  add(e = {}) {
    uo(wn, e);
  }
};
var xa = Ia;
function Ta(e) {
  const t = so(e) + ".";
  return (n, ...o) => {
    const l = xa.messages(), a = En(l, t + n) || En(l, n);
    return It(a) ? a(...o) : a;
  };
}
function qt(e, t) {
  return t ? typeof t == "string" ? ` ${e}--${t}` : Array.isArray(t) ? t.reduce(
    (n, o) => n + qt(e, o),
    ""
  ) : Object.keys(t).reduce(
    (n, o) => n + (t[o] ? qt(e, o) : ""),
    ""
  ) : "";
}
function Da(e) {
  return (t, n) => (t && typeof t != "string" && (n = t, t = ""), t = t ? `${e}__${t}` : e, `${t}${qt(t, n)}`);
}
function W(e) {
  const t = `van-${e}`;
  return [
    t,
    Da(t),
    Ta(t)
  ];
}
const vn = "van-hairline", za = `${vn}--surround`, wa = `${vn}--top-bottom`, Ca = `${vn}-unset--top-bottom`, Ft = "van-haptics-feedback", ja = Symbol("van-form");
function fo(e, {
  args: t = [],
  done: n,
  canceled: o
}) {
  if (e) {
    const l = e.apply(null, t);
    no(l) ? l.then((a) => {
      a ? n() : o && o();
    }).catch(ta) : l ? n() : o && o();
  } else
    n();
}
function se(e) {
  return e.install = (t) => {
    const { name: n } = e;
    n && (t.component(n, e), t.component(so(`-${n}`), e));
  }, e;
}
const vo = Symbol();
function gn(e) {
  const t = Dt(vo, null);
  t && Z(t, (n) => {
    n && e();
  });
}
const Ha = (e, t) => {
  const n = E(), o = () => {
    n.value = he(e).height;
  };
  return Ee(() => {
    if (te(o), t)
      for (let l = 1; l <= 3; l++)
        setTimeout(o, 100 * l);
  }), gn(() => te(o)), Z([st, Ct], o), n;
};
function ye(e) {
  const t = Re();
  t && ce(t.proxy, e);
}
const mn = {
  to: [String, Object],
  url: String,
  replace: Boolean
};
function go({
  to: e,
  url: t,
  replace: n,
  $router: o
}) {
  e && o ? o[n ? "replace" : "push"](e) : t && (n ? location.replace(t) : location.href = t);
}
function mo() {
  const e = Re().proxy;
  return () => go(e);
}
const [Na, Cn] = W("badge"), pa = {
  dot: Boolean,
  max: Y,
  tag: V("div"),
  color: String,
  offset: Array,
  content: Y,
  showZero: q,
  position: V("top-right")
};
var Ba = Q({
  name: Na,
  props: pa,
  setup(e, {
    slots: t
  }) {
    const n = () => {
      if (t.content)
        return !0;
      const {
        content: r,
        showZero: i
      } = e;
      return ae(r) && r !== "" && (i || r !== 0 && r !== "0");
    }, o = () => {
      const {
        dot: r,
        max: i,
        content: u
      } = e;
      if (!r && n())
        return t.content ? t.content() : ae(i) && oo(u) && +u > +i ? `${i}+` : u;
    }, l = (r) => r.startsWith("-") ? r.replace("-", "") : `-${r}`, a = O(() => {
      const r = {
        background: e.color
      };
      if (e.offset) {
        const [i, u] = e.offset, {
          position: s
        } = e, [g, d] = s.split("-");
        t.default ? (typeof u == "number" ? r[g] = ge(g === "top" ? u : -u) : r[g] = g === "top" ? ge(u) : l(u), typeof i == "number" ? r[d] = ge(d === "left" ? i : -i) : r[d] = d === "left" ? ge(i) : l(i)) : (r.marginTop = ge(u), r.marginLeft = ge(i));
      }
      return r;
    }), c = () => {
      if (n() || e.dot)
        return f("div", {
          class: Cn([e.position, {
            dot: e.dot,
            fixed: !!t.default
          }]),
          style: a.value
        }, [o()]);
    };
    return () => {
      if (t.default) {
        const {
          tag: r
        } = e;
        return f(r, {
          class: Cn("wrapper")
        }, {
          default: () => [t.default(), c()]
        });
      }
      return c();
    };
  }
});
const Ao = se(Ba);
let ho = 2e3;
const Ra = () => ++ho, Ja = (e) => {
  ho = e;
}, [yo, Ma] = W("config-provider"), bo = Symbol(yo), Za = {
  tag: V("div"),
  theme: V("light"),
  zIndex: Number,
  themeVars: Object,
  themeVarsDark: Object,
  themeVarsLight: Object,
  iconPrefix: String
};
function Ya(e) {
  const t = {};
  return Object.keys(e).forEach((n) => {
    t[`--van-${ka(n)}`] = e[n];
  }), t;
}
Q({
  name: yo,
  props: Za,
  setup(e, {
    slots: t
  }) {
    const n = O(() => Ya(ce({}, e.themeVars, e.theme === "dark" ? e.themeVarsDark : e.themeVarsLight)));
    if (lt) {
      const o = () => {
        document.documentElement.classList.add(`van-theme-${e.theme}`);
      }, l = (a = e.theme) => {
        document.documentElement.classList.remove(`van-theme-${a}`);
      };
      Z(() => e.theme, (a, c) => {
        c && l(c), o();
      }, {
        immediate: !0
      }), nt(o), Qe(l), zt(l);
    }
    return ot(bo, e), _n(() => {
      e.zIndex !== void 0 && Ja(e.zIndex);
    }), () => f(e.tag, {
      class: Ma(),
      style: n.value
    }, {
      default: () => {
        var o;
        return [(o = t.default) == null ? void 0 : o.call(t)];
      }
    });
  }
});
const [Xa, jn] = W("icon"), Ua = (e) => e == null ? void 0 : e.includes("/"), Ga = {
  dot: Boolean,
  tag: V("i"),
  name: String,
  size: Y,
  badge: Y,
  color: String,
  badgeProps: Object,
  classPrefix: String
};
var Qa = Q({
  name: Xa,
  props: Ga,
  setup(e, {
    slots: t
  }) {
    const n = Dt(bo, null), o = O(() => e.classPrefix || (n == null ? void 0 : n.iconPrefix) || jn());
    return () => {
      const {
        tag: l,
        dot: a,
        name: c,
        size: r,
        badge: i,
        color: u
      } = e, s = Ua(c);
      return f(Ao, fe({
        dot: a,
        tag: l,
        class: [o.value, s ? "" : `${o.value}-${c}`],
        style: {
          color: u,
          fontSize: ge(r)
        },
        content: i
      }, e.badgeProps), {
        default: () => {
          var g;
          return [(g = t.default) == null ? void 0 : g.call(t), s && f("img", {
            class: jn("image"),
            src: c
          }, null)];
        }
      });
    };
  }
});
const Te = se(Qa), [La, _e] = W("loading"), qa = Array(12).fill(null).map((e, t) => f("i", {
  class: _e("line", String(t + 1))
}, null)), Fa = f("svg", {
  class: _e("circular"),
  viewBox: "25 25 50 50"
}, [f("circle", {
  cx: "50",
  cy: "50",
  r: "20",
  fill: "none"
}, null)]), Va = {
  size: Y,
  type: V("circular"),
  color: String,
  vertical: Boolean,
  textSize: Y,
  textColor: String
};
var Wa = Q({
  name: La,
  props: Va,
  setup(e, {
    slots: t
  }) {
    const n = O(() => ce({
      color: e.color
    }, ga(e.size))), o = () => {
      const a = e.type === "spinner" ? qa : Fa;
      return f("span", {
        class: _e("spinner", e.type),
        style: n.value
      }, [t.icon ? t.icon() : a]);
    }, l = () => {
      var a;
      if (t.default)
        return f("span", {
          class: _e("text"),
          style: {
            fontSize: ge(e.textSize),
            color: (a = e.textColor) != null ? a : e.color
          }
        }, [t.default()]);
    };
    return () => {
      const {
        type: a,
        vertical: c
      } = e;
      return f("div", {
        class: _e([a, {
          vertical: c
        }]),
        "aria-live": "polite",
        "aria-busy": !0
      }, [o(), l()]);
    };
  }
});
const ut = se(Wa), [Ka, Je] = W("button"), $a = ce({}, mn, {
  tag: V("button"),
  text: String,
  icon: String,
  type: V("default"),
  size: V("normal"),
  color: String,
  block: Boolean,
  plain: Boolean,
  round: Boolean,
  square: Boolean,
  loading: Boolean,
  hairline: Boolean,
  disabled: Boolean,
  iconPrefix: String,
  nativeType: V("button"),
  loadingSize: Y,
  loadingText: String,
  loadingType: String,
  iconPosition: V("left")
});
var _a = Q({
  name: Ka,
  props: $a,
  emits: ["click"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = mo(), l = () => n.loading ? n.loading() : f(ut, {
      size: e.loadingSize,
      type: e.loadingType,
      class: Je("loading")
    }, null), a = () => {
      if (e.loading)
        return l();
      if (n.icon)
        return f("div", {
          class: Je("icon")
        }, [n.icon()]);
      if (e.icon)
        return f(Te, {
          name: e.icon,
          class: Je("icon"),
          classPrefix: e.iconPrefix
        }, null);
    }, c = () => {
      let u;
      if (e.loading ? u = e.loadingText : u = n.default ? n.default() : e.text, u)
        return f("span", {
          class: Je("text")
        }, [u]);
    }, r = () => {
      const {
        color: u,
        plain: s
      } = e;
      if (u) {
        const g = {
          color: s ? u : "white"
        };
        return s || (g.background = u), u.includes("gradient") ? g.border = 0 : g.borderColor = u, g;
      }
    }, i = (u) => {
      e.loading ? xe(u) : e.disabled || (t("click", u), o());
    };
    return () => {
      const {
        tag: u,
        type: s,
        size: g,
        block: d,
        round: A,
        plain: y,
        square: S,
        loading: m,
        disabled: I,
        hairline: x,
        nativeType: M,
        iconPosition: w
      } = e, T = [Je([s, g, {
        plain: y,
        block: d,
        round: A,
        square: S,
        loading: m,
        disabled: I,
        hairline: x
      }]), {
        [za]: x
      }];
      return f(u, {
        type: M,
        class: T,
        style: r(),
        disabled: I,
        onClick: i
      }, {
        default: () => [f("div", {
          class: Je("content")
        }, [w === "left" && a(), c(), w === "right" && a()])]
      });
    };
  }
});
const An = se(_a), el = {
  // whether to show popup
  show: Boolean,
  // z-index
  zIndex: Y,
  // whether to show overlay
  overlay: q,
  // transition duration
  duration: Y,
  // teleport
  teleport: [String, Object],
  // prevent body scroll
  lockScroll: q,
  // whether to lazy render
  lazyRender: q,
  // callback function before close
  beforeClose: Function,
  // overlay custom style
  overlayStyle: Object,
  // overlay custom class name
  overlayClass: je,
  // Initial rendering animation
  transitionAppear: Boolean,
  // whether to close popup when overlay is clicked
  closeOnClickOverlay: q
};
function tl(e, t) {
  return e > t ? "horizontal" : t > e ? "vertical" : "";
}
function jt() {
  const e = E(0), t = E(0), n = E(0), o = E(0), l = E(0), a = E(0), c = E(""), r = () => c.value === "vertical", i = () => c.value === "horizontal", u = () => {
    n.value = 0, o.value = 0, l.value = 0, a.value = 0, c.value = "";
  };
  return {
    move: (d) => {
      const A = d.touches[0];
      n.value = (A.clientX < 0 ? 0 : A.clientX) - e.value, o.value = A.clientY - t.value, l.value = Math.abs(n.value), a.value = Math.abs(o.value);
      const y = 10;
      (!c.value || l.value < y && a.value < y) && (c.value = tl(l.value, a.value));
    },
    start: (d) => {
      u(), e.value = d.touches[0].clientX, t.value = d.touches[0].clientY;
    },
    reset: u,
    startX: e,
    startY: t,
    deltaX: n,
    deltaY: o,
    offsetX: l,
    offsetY: a,
    direction: c,
    isVertical: r,
    isHorizontal: i
  };
}
let Fe = 0;
const Hn = "van-overflow-hidden";
function nl(e, t) {
  const n = jt(), o = "01", l = "10", a = (s) => {
    n.move(s);
    const g = n.deltaY.value > 0 ? l : o, d = ro(
      s.target,
      e.value
    ), { scrollHeight: A, offsetHeight: y, scrollTop: S } = d;
    let m = "11";
    S === 0 ? m = y >= A ? "00" : "01" : S + y >= A && (m = "10"), m !== "11" && n.isVertical() && !(parseInt(m, 2) & parseInt(g, 2)) && xe(s, !0);
  }, c = () => {
    document.addEventListener("touchstart", n.start), document.addEventListener("touchmove", a, { passive: !1 }), Fe || document.body.classList.add(Hn), Fe++;
  }, r = () => {
    Fe && (document.removeEventListener("touchstart", n.start), document.removeEventListener("touchmove", a), Fe--, Fe || document.body.classList.remove(Hn));
  }, i = () => t() && c(), u = () => t() && r();
  rt(i), Qe(u), zt(u), Z(t, (s) => {
    s ? c() : r();
  });
}
function ko(e) {
  const t = E(!1);
  return Z(
    e,
    (n) => {
      n && (t.value = n);
    },
    { immediate: !0 }
  ), (n) => () => t.value ? n() : null;
}
const [ol, al] = W("overlay"), ll = {
  show: Boolean,
  zIndex: Y,
  duration: Y,
  className: je,
  lockScroll: q,
  lazyRender: q,
  customStyle: Object
};
var il = Q({
  name: ol,
  props: ll,
  setup(e, {
    slots: t
  }) {
    const n = E(), o = ko(() => e.show || !e.lazyRender), l = (c) => {
      e.lockScroll && xe(c, !0);
    }, a = o(() => {
      var c;
      const r = ce(co(e.zIndex), e.customStyle);
      return ae(e.duration) && (r.animationDuration = `${e.duration}s`), an(f("div", {
        ref: n,
        style: r,
        class: [al(), e.className]
      }, [(c = t.default) == null ? void 0 : c.call(t)]), [[ln, e.show]]);
    });
    return ze("touchmove", l, {
      target: n
    }), () => f(eo, {
      name: "van-fade",
      appear: !0
    }, {
      default: a
    });
  }
});
const rl = se(il), cl = ce({}, el, {
  round: Boolean,
  position: V("center"),
  closeIcon: V("cross"),
  closeable: Boolean,
  transition: String,
  iconPrefix: String,
  closeOnPopstate: Boolean,
  closeIconPosition: V("top-right"),
  safeAreaInsetTop: Boolean,
  safeAreaInsetBottom: Boolean
}), [sl, Nn] = W("popup");
var ul = Q({
  name: sl,
  inheritAttrs: !1,
  props: cl,
  emits: ["open", "close", "opened", "closed", "keydown", "update:show", "clickOverlay", "clickCloseIcon"],
  setup(e, {
    emit: t,
    attrs: n,
    slots: o
  }) {
    let l, a;
    const c = E(), r = E(), i = ko(() => e.show || !e.lazyRender), u = O(() => {
      const T = {
        zIndex: c.value
      };
      if (ae(e.duration)) {
        const B = e.position === "center" ? "animationDuration" : "transitionDuration";
        T[B] = `${e.duration}s`;
      }
      return T;
    }), s = () => {
      l || (l = !0, c.value = e.zIndex !== void 0 ? +e.zIndex : Ra(), t("open"));
    }, g = () => {
      l && fo(e.beforeClose, {
        done() {
          l = !1, t("close"), t("update:show", !1);
        }
      });
    }, d = (T) => {
      t("clickOverlay", T), e.closeOnClickOverlay && g();
    }, A = () => {
      if (e.overlay)
        return f(rl, {
          show: e.show,
          class: e.overlayClass,
          zIndex: c.value,
          duration: e.duration,
          customStyle: e.overlayStyle,
          role: e.closeOnClickOverlay ? "button" : void 0,
          tabindex: e.closeOnClickOverlay ? 0 : void 0,
          onClick: d
        }, {
          default: o["overlay-content"]
        });
    }, y = (T) => {
      t("clickCloseIcon", T), g();
    }, S = () => {
      if (e.closeable)
        return f(Te, {
          role: "button",
          tabindex: 0,
          name: e.closeIcon,
          class: [Nn("close-icon", e.closeIconPosition), Ft],
          classPrefix: e.iconPrefix,
          onClick: y
        }, null);
    }, m = () => t("opened"), I = () => t("closed"), x = (T) => t("keydown", T), M = i(() => {
      var T;
      const {
        round: B,
        position: K,
        safeAreaInsetTop: ne,
        safeAreaInsetBottom: $
      } = e;
      return an(f("div", fe({
        ref: r,
        style: u.value,
        role: "dialog",
        tabindex: 0,
        class: [Nn({
          round: B,
          [K]: K
        }), {
          "van-safe-area-top": ne,
          "van-safe-area-bottom": $
        }],
        onKeydown: x
      }, n), [(T = o.default) == null ? void 0 : T.call(o), S()]), [[ln, e.show]]);
    }), w = () => {
      const {
        position: T,
        transition: B,
        transitionAppear: K
      } = e, ne = T === "center" ? "van-fade" : `van-popup-slide-${T}`;
      return f(eo, {
        name: B || ne,
        appear: K,
        onAfterEnter: m,
        onAfterLeave: I
      }, {
        default: M
      });
    };
    return Z(() => e.show, (T) => {
      T && !l && (s(), n.tabindex === 0 && te(() => {
        var B;
        (B = r.value) == null || B.focus();
      })), !T && l && (l = !1, t("close"));
    }), ye({
      popupRef: r
    }), nl(r, () => e.show && e.lockScroll), ze("popstate", () => {
      e.closeOnPopstate && (g(), a = !1);
    }), Ee(() => {
      e.show && s();
    }), nt(() => {
      a && (t("update:show", !0), a = !1);
    }), Qe(() => {
      e.show && e.teleport && (g(), a = !0);
    }), ot(vo, () => e.show), () => e.teleport ? f(Qo, {
      to: e.teleport
    }, {
      default: () => [A(), w()]
    }) : f(Ne, null, [A(), w()]);
  }
});
const hn = se(ul), [dl, Ce, pn] = W("picker"), Po = (e) => e.find((t) => !t.disabled) || e[0];
function fl(e, t) {
  const n = e[0];
  if (n) {
    if (Array.isArray(n))
      return "multiple";
    if (t.children in n)
      return "cascade";
  }
  return "default";
}
function Vt(e, t) {
  t = Xe(t, 0, e.length);
  for (let n = t; n < e.length; n++)
    if (!e[n].disabled)
      return n;
  for (let n = t - 1; n >= 0; n--)
    if (!e[n].disabled)
      return n;
  return 0;
}
const Bn = (e, t, n) => t !== void 0 && !!e.find((o) => o[n.value] === t);
function Wt(e, t, n) {
  const o = e.findIndex((a) => a[n.value] === t), l = Vt(e, o);
  return e[l];
}
function vl(e, t, n) {
  const o = [];
  let l = {
    [t.children]: e
  }, a = 0;
  for (; l && l[t.children]; ) {
    const c = l[t.children], r = n.value[a];
    if (l = ae(r) ? Wt(c, r, t) : void 0, !l && c.length) {
      const i = Po(c)[t.value];
      l = Wt(c, i, t);
    }
    a++, o.push(c);
  }
  return o;
}
function gl(e) {
  const { transform: t } = window.getComputedStyle(e), n = t.slice(7, t.length - 1).split(", ")[5];
  return Number(n);
}
function ml(e) {
  return ce(
    {
      text: "text",
      value: "value",
      children: "children"
    },
    e
  );
}
const Rn = 200, Jn = 300, Al = 15, [Oo, Jt] = W("picker-column"), Eo = Symbol(Oo);
var hl = Q({
  name: Oo,
  props: {
    value: Y,
    fields: Oe(Object),
    options: tt(),
    readonly: Boolean,
    allowHtml: Boolean,
    optionHeight: Oe(Number),
    swipeDuration: Oe(Y),
    visibleOptionNum: Oe(Y)
  },
  emits: ["change", "clickOption", "scrollInto"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    let o, l, a, c, r;
    const i = E(), u = E(), s = E(0), g = E(0), d = jt(), A = () => e.options.length, y = () => e.optionHeight * (+e.visibleOptionNum - 1) / 2, S = (k) => {
      const h = Vt(e.options, k), P = -h * e.optionHeight, z = () => {
        const U = e.options[h][e.fields.value];
        U !== e.value && t("change", U);
      };
      o && P !== s.value ? r = z : z(), s.value = P;
    }, m = () => e.readonly || !e.options.length, I = (k) => {
      o || m() || (r = null, g.value = Rn, S(k), t("clickOption", e.options[k]));
    }, x = (k) => Xe(Math.round(-k / e.optionHeight), 0, A() - 1), M = O(() => x(s.value)), w = (k, h) => {
      const P = Math.abs(k / h);
      k = s.value + P / 3e-3 * (k < 0 ? -1 : 1);
      const z = x(k);
      g.value = +e.swipeDuration, S(z);
    }, T = () => {
      o = !1, g.value = 0, r && (r(), r = null);
    }, B = (k) => {
      if (!m()) {
        if (d.start(k), o) {
          const h = gl(u.value);
          s.value = Math.min(0, h - y());
        }
        g.value = 0, l = s.value, a = Date.now(), c = l, r = null;
      }
    }, K = (k) => {
      if (m())
        return;
      d.move(k), d.isVertical() && (o = !0, xe(k, !0));
      const h = Xe(l + d.deltaY.value, -(A() * e.optionHeight), e.optionHeight), P = x(h);
      P !== M.value && t("scrollInto", e.options[P]), s.value = h;
      const z = Date.now();
      z - a > Jn && (a = z, c = h);
    }, ne = () => {
      if (m())
        return;
      const k = s.value - c, h = Date.now() - a;
      if (h < Jn && Math.abs(k) > Al) {
        w(k, h);
        return;
      }
      const z = x(s.value);
      g.value = Rn, S(z), setTimeout(() => {
        o = !1;
      }, 0);
    }, $ = () => {
      const k = {
        height: `${e.optionHeight}px`
      };
      return e.options.map((h, P) => {
        const z = h[e.fields.text], {
          disabled: U
        } = h, b = h[e.fields.value], j = {
          role: "button",
          style: k,
          tabindex: U ? -1 : 0,
          class: [Jt("item", {
            disabled: U,
            selected: b === e.value
          }), h.className],
          onClick: () => I(P)
        }, R = {
          class: "van-ellipsis",
          [e.allowHtml ? "innerHTML" : "textContent"]: z
        };
        return f("li", j, [n.option ? n.option(h, P) : f("div", R, null)]);
      });
    };
    return qe(Eo), ye({
      stopMomentum: T
    }), _n(() => {
      const k = e.options.findIndex((z) => z[e.fields.value] === e.value), P = -Vt(e.options, k) * e.optionHeight;
      s.value = P;
    }), ze("touchmove", K, {
      target: i
    }), () => f("div", {
      ref: i,
      class: Jt(),
      onTouchstartPassive: B,
      onTouchend: ne,
      onTouchcancel: ne
    }, [f("ul", {
      ref: u,
      style: {
        transform: `translate3d(0, ${s.value + y()}px, 0)`,
        transitionDuration: `${g.value}ms`,
        transitionProperty: g.value ? "all" : "none"
      },
      class: Jt("wrapper"),
      onTransitionend: T
    }, [$()])]);
  }
});
const [yl] = W("picker-toolbar"), Ht = {
  title: String,
  cancelButtonText: String,
  confirmButtonText: String
}, So = ["cancel", "confirm", "title", "toolbar"], bl = Object.keys(Ht);
var Io = Q({
  name: yl,
  props: Ht,
  emits: ["confirm", "cancel"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = () => {
      if (n.title)
        return n.title();
      if (e.title)
        return f("div", {
          class: [Ce("title"), "van-ellipsis"]
        }, [e.title]);
    }, l = () => t("cancel"), a = () => t("confirm"), c = () => {
      const i = e.cancelButtonText || pn("cancel");
      return f("button", {
        type: "button",
        class: [Ce("cancel"), Ft],
        onClick: l
      }, [n.cancel ? n.cancel() : i]);
    }, r = () => {
      const i = e.confirmButtonText || pn("confirm");
      return f("button", {
        type: "button",
        class: [Ce("confirm"), Ft],
        onClick: a
      }, [n.confirm ? n.confirm() : i]);
    };
    return () => f("div", {
      class: Ce("toolbar")
    }, [n.toolbar ? n.toolbar() : [c(), o(), r()]]);
  }
});
const kl = (e, t) => {
  const n = E(e());
  return Z(e, (o) => {
    o !== n.value && (n.value = o);
  }), Z(n, (o) => {
    o !== e() && t(o);
  }), n;
};
function Pl(e, t, n) {
  let o, l = 0;
  const a = e.scrollLeft, c = n === 0 ? 1 : Math.round(n * 1e3 / 16);
  function r() {
    lo(o);
  }
  function i() {
    e.scrollLeft += (t - a) / c, ++l < c && (o = Be(i));
  }
  return i(), r;
}
function Ol(e, t, n, o) {
  let l, a = ct(e);
  const c = a < t, r = n === 0 ? 1 : Math.round(n * 1e3 / 16), i = (t - a) / r;
  function u() {
    lo(l);
  }
  function s() {
    a += i, (c && a > t || !c && a < t) && (a = t), xt(e, a), c && a < t || !c && a > t ? l = Be(s) : o && (l = Be(o));
  }
  return s(), u;
}
let El = 0;
function yn() {
  const e = Re(), { name: t = "unknown" } = (e == null ? void 0 : e.type) || {};
  return process.env.NODE_ENV === "test" ? t : `${t}-${++El}`;
}
function xo() {
  const e = E([]), t = [];
  return Lo(() => {
    e.value = [];
  }), [e, (o) => (t[o] || (t[o] = (l) => {
    e.value[o] = l;
  }), t[o])];
}
function To(e, t) {
  if (!lt || !window.IntersectionObserver)
    return;
  const n = new IntersectionObserver(
    (a) => {
      t(a[0].intersectionRatio > 0);
    },
    { root: document.body }
  ), o = () => {
    e.value && n.observe(e.value);
  }, l = () => {
    e.value && n.unobserve(e.value);
  };
  Qe(l), zt(l), rt(o);
}
const [Sl, Il] = W("sticky"), xl = {
  zIndex: Y,
  position: V("top"),
  container: Object,
  offsetTop: le(0),
  offsetBottom: le(0)
};
var Tl = Q({
  name: Sl,
  props: xl,
  emits: ["scroll", "change"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = E(), l = wt(o), a = De({
      fixed: !1,
      width: 0,
      // root width
      height: 0,
      // root height
      transform: 0
    }), c = E(!1), r = O(() => fn(e.position === "top" ? e.offsetTop : e.offsetBottom)), i = O(() => {
      if (c.value)
        return;
      const {
        fixed: d,
        height: A,
        width: y
      } = a;
      if (d)
        return {
          width: `${y}px`,
          height: `${A}px`
        };
    }), u = O(() => {
      if (!a.fixed || c.value)
        return;
      const d = ce(co(e.zIndex), {
        width: `${a.width}px`,
        height: `${a.height}px`,
        [e.position]: `${r.value}px`
      });
      return a.transform && (d.transform = `translate3d(0, ${a.transform}px, 0)`), d;
    }), s = (d) => t("scroll", {
      scrollTop: d,
      isFixed: a.fixed
    }), g = () => {
      if (!o.value || Ge(o))
        return;
      const {
        container: d,
        position: A
      } = e, y = he(o), S = ct(window);
      if (a.width = y.width, a.height = y.height, A === "top")
        if (d) {
          const m = he(d), I = m.bottom - r.value - a.height;
          a.fixed = r.value > y.top && m.bottom > 0, a.transform = I < 0 ? I : 0;
        } else
          a.fixed = r.value > y.top;
      else {
        const {
          clientHeight: m
        } = document.documentElement;
        if (d) {
          const I = he(d), x = m - I.top - r.value - a.height;
          a.fixed = m - r.value < y.bottom && m > I.top, a.transform = x < 0 ? -x : 0;
        } else
          a.fixed = m - r.value < y.bottom;
      }
      s(S);
    };
    return Z(() => a.fixed, (d) => t("change", d)), ze("scroll", g, {
      target: l,
      passive: !0
    }), To(o, g), Z([st, Ct], () => {
      !o.value || Ge(o) || !a.fixed || (c.value = !0, te(() => {
        const d = he(o);
        a.width = d.width, a.height = d.height, c.value = !1;
      }));
    }), () => {
      var d;
      return f("div", {
        ref: o,
        style: i.value
      }, [f("div", {
        class: Il({
          fixed: a.fixed && !c.value
        }),
        style: u.value
      }, [(d = n.default) == null ? void 0 : d.call(n)])]);
    };
  }
});
const Dl = se(Tl), [Do, mt] = W("swipe"), zl = {
  loop: q,
  width: Y,
  height: Y,
  vertical: Boolean,
  autoplay: le(0),
  duration: le(500),
  touchable: q,
  lazyRender: Boolean,
  initialSwipe: le(0),
  indicatorColor: String,
  showIndicators: q,
  stopPropagation: q
}, zo = Symbol(Do);
var wl = Q({
  name: Do,
  props: zl,
  emits: ["change", "dragStart", "dragEnd"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = E(), l = E(), a = De({
      rect: null,
      width: 0,
      height: 0,
      offset: 0,
      active: 0,
      swiping: !1
    });
    let c = !1;
    const r = jt(), {
      children: i,
      linkChildren: u
    } = it(zo), s = O(() => i.length), g = O(() => a[e.vertical ? "height" : "width"]), d = O(() => e.vertical ? r.deltaY.value : r.deltaX.value), A = O(() => a.rect ? (e.vertical ? a.rect.height : a.rect.width) - g.value * s.value : 0), y = O(() => g.value ? Math.ceil(Math.abs(A.value) / g.value) : s.value), S = O(() => s.value * g.value), m = O(() => (a.active + s.value) % s.value), I = O(() => {
      const H = e.vertical ? "vertical" : "horizontal";
      return r.direction.value === H;
    }), x = O(() => {
      const H = {
        transitionDuration: `${a.swiping ? 0 : e.duration}ms`,
        transform: `translate${e.vertical ? "Y" : "X"}(${a.offset}px)`
      };
      if (g.value) {
        const p = e.vertical ? "height" : "width", X = e.vertical ? "width" : "height";
        H[p] = `${S.value}px`, H[X] = e[X] ? `${e[X]}px` : "";
      }
      return H;
    }), M = (H) => {
      const {
        active: p
      } = a;
      return H ? e.loop ? Xe(p + H, -1, s.value) : Xe(p + H, 0, y.value) : p;
    }, w = (H, p = 0) => {
      let X = H * g.value;
      e.loop || (X = Math.min(X, -A.value));
      let ue = p - X;
      return e.loop || (ue = Xe(ue, A.value, 0)), ue;
    }, T = ({
      pace: H = 0,
      offset: p = 0,
      emitChange: X
    }) => {
      if (s.value <= 1)
        return;
      const {
        active: ue
      } = a, D = M(H), v = w(D, p);
      if (e.loop) {
        if (i[0] && v !== A.value) {
          const C = v < A.value;
          i[0].setOffset(C ? S.value : 0);
        }
        if (i[s.value - 1] && v !== 0) {
          const C = v > 0;
          i[s.value - 1].setOffset(C ? -S.value : 0);
        }
      }
      a.active = D, a.offset = v, X && D !== ue && t("change", m.value);
    }, B = () => {
      a.swiping = !0, a.active <= -1 ? T({
        pace: s.value
      }) : a.active >= s.value && T({
        pace: -s.value
      });
    }, K = () => {
      B(), r.reset(), Ke(() => {
        a.swiping = !1, T({
          pace: -1,
          emitChange: !0
        });
      });
    }, ne = () => {
      B(), r.reset(), Ke(() => {
        a.swiping = !1, T({
          pace: 1,
          emitChange: !0
        });
      });
    };
    let $;
    const k = () => clearTimeout($), h = () => {
      k(), +e.autoplay > 0 && s.value > 1 && ($ = setTimeout(() => {
        ne(), h();
      }, +e.autoplay));
    }, P = (H = +e.initialSwipe) => {
      if (!o.value)
        return;
      const p = () => {
        var X, ue;
        if (!Ge(o)) {
          const D = {
            width: o.value.offsetWidth,
            height: o.value.offsetHeight
          };
          a.rect = D, a.width = +((X = e.width) != null ? X : D.width), a.height = +((ue = e.height) != null ? ue : D.height);
        }
        s.value && (H = Math.min(s.value - 1, H), H === -1 && (H = s.value - 1)), a.active = H, a.swiping = !0, a.offset = w(H), i.forEach((D) => {
          D.setOffset(0);
        }), h();
      };
      Ge(o) ? te().then(p) : p();
    }, z = () => P(a.active);
    let U;
    const b = (H) => {
      !e.touchable || // avoid resetting position on multi-finger touch
      H.touches.length > 1 || (r.start(H), c = !1, U = Date.now(), k(), B());
    }, j = (H) => {
      e.touchable && a.swiping && (r.move(H), I.value && (!e.loop && (a.active === 0 && d.value > 0 || a.active === s.value - 1 && d.value < 0) || (xe(H, e.stopPropagation), T({
        offset: d.value
      }), c || (t("dragStart", {
        index: m.value
      }), c = !0))));
    }, R = () => {
      if (!e.touchable || !a.swiping)
        return;
      const H = Date.now() - U, p = d.value / H;
      if ((Math.abs(p) > 0.25 || Math.abs(d.value) > g.value / 2) && I.value) {
        const ue = e.vertical ? r.offsetY.value : r.offsetX.value;
        let D = 0;
        e.loop ? D = ue > 0 ? d.value > 0 ? -1 : 1 : 0 : D = -Math[d.value > 0 ? "ceil" : "floor"](d.value / g.value), T({
          pace: D,
          emitChange: !0
        });
      } else
        d.value && T({
          pace: 0
        });
      c = !1, a.swiping = !1, t("dragEnd", {
        index: m.value
      }), h();
    }, G = (H, p = {}) => {
      B(), r.reset(), Ke(() => {
        let X;
        e.loop && H === s.value ? X = a.active === 0 ? 0 : H : X = H % s.value, p.immediate ? Ke(() => {
          a.swiping = !1;
        }) : a.swiping = !1, T({
          pace: X - a.active,
          emitChange: !0
        });
      });
    }, ie = (H, p) => {
      const X = p === m.value, ue = X ? {
        backgroundColor: e.indicatorColor
      } : void 0;
      return f("i", {
        style: ue,
        class: mt("indicator", {
          active: X
        })
      }, null);
    }, _ = () => {
      if (n.indicator)
        return n.indicator({
          active: m.value,
          total: s.value
        });
      if (e.showIndicators && s.value > 1)
        return f("div", {
          class: mt("indicators", {
            vertical: e.vertical
          })
        }, [Array(s.value).fill("").map(ie)]);
    };
    return ye({
      prev: K,
      next: ne,
      state: a,
      resize: z,
      swipeTo: G
    }), u({
      size: g,
      props: e,
      count: s,
      activeIndicator: m
    }), Z(() => e.initialSwipe, (H) => P(+H)), Z(s, () => P(a.active)), Z(() => e.autoplay, h), Z([st, Ct, () => e.width, () => e.height], z), Z(sa(), (H) => {
      H === "visible" ? h() : k();
    }), Ee(P), nt(() => P(a.active)), gn(() => P(a.active)), Qe(k), zt(k), ze("touchmove", j, {
      target: l
    }), () => {
      var H;
      return f("div", {
        ref: o,
        class: mt()
      }, [f("div", {
        ref: l,
        style: x.value,
        class: mt("track", {
          vertical: e.vertical
        }),
        onTouchstartPassive: b,
        onTouchend: R,
        onTouchcancel: R
      }, [(H = n.default) == null ? void 0 : H.call(n)]), _()]);
    };
  }
});
const Cl = se(wl), [jl, Mn] = W("tabs");
var Hl = Q({
  name: jl,
  props: {
    count: Oe(Number),
    inited: Boolean,
    animated: Boolean,
    duration: Oe(Y),
    swipeable: Boolean,
    lazyRender: Boolean,
    currentIndex: Oe(Number)
  },
  emits: ["change"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = E(), l = (r) => t("change", r), a = () => {
      var r;
      const i = (r = n.default) == null ? void 0 : r.call(n);
      return e.animated || e.swipeable ? f(Cl, {
        ref: o,
        loop: !1,
        class: Mn("track"),
        duration: +e.duration * 1e3,
        touchable: e.swipeable,
        lazyRender: e.lazyRender,
        showIndicators: !1,
        onChange: l
      }, {
        default: () => [i]
      }) : i;
    }, c = (r) => {
      const i = o.value;
      i && i.state.active !== r && i.swipeTo(r, {
        immediate: !e.inited
      });
    };
    return Z(() => e.currentIndex, c), Ee(() => {
      c(e.currentIndex);
    }), ye({
      swipeRef: o
    }), () => f("div", {
      class: Mn("content", {
        animated: e.animated || e.swipeable
      })
    }, [a()]);
  }
});
const [wo, At] = W("tabs"), Nl = {
  type: V("line"),
  color: String,
  border: Boolean,
  sticky: Boolean,
  shrink: Boolean,
  active: le(0),
  duration: le(0.3),
  animated: Boolean,
  ellipsis: q,
  swipeable: Boolean,
  scrollspy: Boolean,
  offsetTop: le(0),
  background: String,
  lazyRender: q,
  lineWidth: Y,
  lineHeight: Y,
  beforeChange: Function,
  swipeThreshold: le(5),
  titleActiveColor: String,
  titleInactiveColor: String
}, Co = Symbol(wo);
var pl = Q({
  name: wo,
  props: Nl,
  emits: ["change", "scroll", "rendered", "clickTab", "update:active"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    let o, l, a, c, r;
    const i = E(), u = E(), s = E(), g = E(), d = yn(), A = wt(i), [y, S] = xo(), {
      children: m,
      linkChildren: I
    } = it(Co), x = De({
      inited: !1,
      position: "",
      lineStyle: {},
      currentIndex: -1
    }), M = O(() => m.length > +e.swipeThreshold || !e.ellipsis || e.shrink), w = O(() => ({
      borderColor: e.color,
      background: e.background
    })), T = (D, v) => {
      var C;
      return (C = D.name) != null ? C : v;
    }, B = O(() => {
      const D = m[x.currentIndex];
      if (D)
        return T(D, x.currentIndex);
    }), K = O(() => fn(e.offsetTop)), ne = O(() => e.sticky ? K.value + o : 0), $ = (D) => {
      const v = u.value, C = y.value;
      if (!M.value || !v || !C || !C[x.currentIndex])
        return;
      const N = C[x.currentIndex].$el, J = N.offsetLeft - (v.offsetWidth - N.offsetWidth) / 2;
      c && c(), c = Pl(v, J, D ? 0 : +e.duration);
    }, k = () => {
      const D = x.inited;
      te(() => {
        const v = y.value;
        if (!v || !v[x.currentIndex] || e.type !== "line" || Ge(i.value))
          return;
        const C = v[x.currentIndex].$el, {
          lineWidth: N,
          lineHeight: J
        } = e, L = C.offsetLeft + C.offsetWidth / 2, ee = {
          width: ge(N),
          backgroundColor: e.color,
          transform: `translateX(${L}px) translateX(-50%)`
        };
        if (D && (ee.transitionDuration = `${e.duration}s`), ae(J)) {
          const ve = ge(J);
          ee.height = ve, ee.borderRadius = ve;
        }
        x.lineStyle = ee;
      });
    }, h = (D) => {
      const v = D < x.currentIndex ? -1 : 1;
      for (; D >= 0 && D < m.length; ) {
        if (!m[D].disabled)
          return D;
        D += v;
      }
    }, P = (D, v) => {
      const C = h(D);
      if (!ae(C))
        return;
      const N = m[C], J = T(N, C), L = x.currentIndex !== null;
      x.currentIndex !== C && (x.currentIndex = C, v || $(), k()), J !== e.active && (t("update:active", J), L && t("change", J, N.title)), a && !e.scrollspy && dn(Math.ceil(Tn(i.value) - K.value));
    }, z = (D, v) => {
      const C = m.find((J, L) => T(J, L) === D), N = C ? m.indexOf(C) : 0;
      P(N, v);
    }, U = (D = !1) => {
      if (e.scrollspy) {
        const v = m[x.currentIndex].$el;
        if (v && A.value) {
          const C = Tn(v, A.value) - ne.value;
          l = !0, r && r(), r = Ol(A.value, C, D ? 0 : +e.duration, () => {
            l = !1;
          });
        }
      }
    }, b = (D, v, C) => {
      const {
        title: N,
        disabled: J
      } = m[v], L = T(m[v], v);
      J || (fo(e.beforeChange, {
        args: [L],
        done: () => {
          P(v), U();
        }
      }), go(D)), t("clickTab", {
        name: L,
        title: N,
        event: C,
        disabled: J
      });
    }, j = (D) => {
      a = D.isFixed, t("scroll", D);
    }, R = (D) => {
      te(() => {
        z(D), U(!0);
      });
    }, G = () => {
      for (let D = 0; D < m.length; D++) {
        const {
          top: v
        } = he(m[D].$el);
        if (v > ne.value)
          return D === 0 ? 0 : D - 1;
      }
      return m.length - 1;
    }, ie = () => {
      if (e.scrollspy && !l) {
        const D = G();
        P(D);
      }
    }, _ = () => {
      if (e.type === "line" && m.length)
        return f("div", {
          class: At("line"),
          style: x.lineStyle
        }, null);
    }, H = () => {
      var D, v, C;
      const {
        type: N,
        border: J,
        sticky: L
      } = e, ee = [f("div", {
        ref: L ? void 0 : s,
        class: [At("wrap"), {
          [wa]: N === "line" && J
        }]
      }, [f("div", {
        ref: u,
        role: "tablist",
        class: At("nav", [N, {
          shrink: e.shrink,
          complete: M.value
        }]),
        style: w.value,
        "aria-orientation": "horizontal"
      }, [(D = n["nav-left"]) == null ? void 0 : D.call(n), m.map((ve) => ve.renderTitle(b)), _(), (v = n["nav-right"]) == null ? void 0 : v.call(n)])]), (C = n["nav-bottom"]) == null ? void 0 : C.call(n)];
      return L ? f("div", {
        ref: s
      }, [ee]) : ee;
    }, p = () => {
      k(), te(() => {
        var D, v;
        $(!0), (v = (D = g.value) == null ? void 0 : D.swipeRef.value) == null || v.resize();
      });
    };
    Z(() => [e.color, e.duration, e.lineWidth, e.lineHeight], k), Z(st, p), Z(() => e.active, (D) => {
      D !== B.value && z(D);
    }), Z(() => m.length, () => {
      x.inited && (z(e.active), k(), te(() => {
        $(!0);
      }));
    });
    const X = () => {
      z(e.active, !0), te(() => {
        x.inited = !0, s.value && (o = he(s.value).height), $(!0);
      });
    }, ue = (D, v) => t("rendered", D, v);
    return ye({
      resize: p,
      scrollTo: R
    }), nt(k), gn(k), rt(X), To(i, k), ze("scroll", ie, {
      target: A,
      passive: !0
    }), I({
      id: d,
      props: e,
      setLine: k,
      scrollable: M,
      onRendered: ue,
      currentName: B,
      setTitleRefs: S,
      scrollIntoView: $
    }), () => f("div", {
      ref: i,
      class: At([e.type])
    }, [e.sticky ? f(Dl, {
      container: i.value,
      offsetTop: K.value,
      onScroll: j
    }, {
      default: () => [H()]
    }) : H(), f(Hl, {
      ref: g,
      count: m.length,
      inited: x.inited,
      animated: e.animated,
      duration: e.duration,
      swipeable: e.swipeable,
      lazyRender: e.lazyRender,
      currentIndex: x.currentIndex,
      onChange: P
    }, {
      default: () => {
        var D;
        return [(D = n.default) == null ? void 0 : D.call(n)];
      }
    })]);
  }
});
const jo = Symbol(), Bl = () => Dt(jo, null), [Rl, Zn] = W("tab"), Jl = Q({
  name: Rl,
  props: {
    id: String,
    dot: Boolean,
    type: String,
    color: String,
    title: String,
    badge: Y,
    shrink: Boolean,
    isActive: Boolean,
    disabled: Boolean,
    controls: String,
    scrollable: Boolean,
    activeColor: String,
    inactiveColor: String,
    showZeroBadge: q
  },
  setup(e, {
    slots: t
  }) {
    const n = O(() => {
      const l = {}, {
        type: a,
        color: c,
        disabled: r,
        isActive: i,
        activeColor: u,
        inactiveColor: s
      } = e;
      c && a === "card" && (l.borderColor = c, r || (i ? l.backgroundColor = c : l.color = c));
      const d = i ? u : s;
      return d && (l.color = d), l;
    }), o = () => {
      const l = f("span", {
        class: Zn("text", {
          ellipsis: !e.scrollable
        })
      }, [t.title ? t.title() : e.title]);
      return e.dot || ae(e.badge) && e.badge !== "" ? f(Ao, {
        dot: e.dot,
        content: e.badge,
        showZero: e.showZeroBadge
      }, {
        default: () => [l]
      }) : l;
    };
    return () => f("div", {
      id: e.id,
      role: "tab",
      class: [Zn([e.type, {
        grow: e.scrollable && !e.shrink,
        shrink: e.shrink,
        active: e.isActive,
        disabled: e.disabled
      }])],
      style: n.value,
      tabindex: e.disabled ? void 0 : e.isActive ? 0 : -1,
      "aria-selected": e.isActive,
      "aria-disabled": e.disabled || void 0,
      "aria-controls": e.controls
    }, [o()]);
  }
}), [Ml, Zl] = W("swipe-item");
var Yl = Q({
  name: Ml,
  setup(e, {
    slots: t
  }) {
    let n;
    const o = De({
      offset: 0,
      inited: !1,
      mounted: !1
    }), {
      parent: l,
      index: a
    } = qe(zo);
    if (!l) {
      process.env.NODE_ENV !== "production" && console.error("[Vant] <SwipeItem> must be a child component of <Swipe>.");
      return;
    }
    const c = O(() => {
      const u = {}, {
        vertical: s
      } = l.props;
      return l.size.value && (u[s ? "height" : "width"] = `${l.size.value}px`), o.offset && (u.transform = `translate${s ? "Y" : "X"}(${o.offset}px)`), u;
    }), r = O(() => {
      const {
        loop: u,
        lazyRender: s
      } = l.props;
      if (!s || n)
        return !0;
      if (!o.mounted)
        return !1;
      const g = l.activeIndicator.value, d = l.count.value - 1, A = g === 0 && u ? d : g - 1, y = g === d && u ? 0 : g + 1;
      return n = a.value === g || a.value === A || a.value === y, n;
    }), i = (u) => {
      o.offset = u;
    };
    return Ee(() => {
      te(() => {
        o.mounted = !0;
      });
    }), ye({
      setOffset: i
    }), () => {
      var u;
      return f("div", {
        class: Zl(),
        style: c.value
      }, [r.value ? (u = t.default) == null ? void 0 : u.call(t) : null]);
    };
  }
});
const Xl = se(Yl), [Ul, Mt] = W("tab"), Gl = ce({}, mn, {
  dot: Boolean,
  name: Y,
  badge: Y,
  title: String,
  disabled: Boolean,
  titleClass: je,
  titleStyle: [String, Object],
  showZeroBadge: q
});
var Ql = Q({
  name: Ul,
  props: Gl,
  setup(e, {
    slots: t
  }) {
    const n = yn(), o = E(!1), l = Re(), {
      parent: a,
      index: c
    } = qe(Co);
    if (!a) {
      process.env.NODE_ENV !== "production" && console.error("[Vant] <Tab> must be a child component of <Tabs>.");
      return;
    }
    const r = () => {
      var d;
      return (d = e.name) != null ? d : c.value;
    }, i = () => {
      o.value = !0, a.props.lazyRender && te(() => {
        a.onRendered(r(), e.title);
      });
    }, u = O(() => {
      const d = r() === a.currentName.value;
      return d && !o.value && i(), d;
    }), s = (d) => f(Jl, fe({
      key: n,
      id: `${a.id}-${c.value}`,
      ref: a.setTitleRefs(c.value),
      style: e.titleStyle,
      class: e.titleClass,
      isActive: u.value,
      controls: n,
      scrollable: a.scrollable.value,
      activeColor: a.props.titleActiveColor,
      inactiveColor: a.props.titleInactiveColor,
      onClick: (A) => d(l.proxy, c.value, A)
    }, Ie(a.props, ["type", "color", "shrink"]), Ie(e, ["dot", "badge", "title", "disabled", "showZeroBadge"])), {
      title: t.title
    }), g = E(!u.value);
    return Z(u, (d) => {
      d ? g.value = !1 : Ke(() => {
        g.value = !0;
      });
    }), Z(() => e.title, () => {
      a.setLine(), a.scrollIntoView();
    }), ot(jo, u), ye({
      id: n,
      renderTitle: s
    }), () => {
      var d;
      const A = `${a.id}-${c.value}`, {
        animated: y,
        swipeable: S,
        scrollspy: m,
        lazyRender: I
      } = a.props;
      if (!t.default && !y)
        return;
      const x = m || u.value;
      if (y || S)
        return f(Xl, {
          id: n,
          role: "tabpanel",
          class: Mt("panel-wrapper", {
            inactive: g.value
          }),
          tabindex: u.value ? 0 : -1,
          "aria-hidden": !u.value,
          "aria-labelledby": A
        }, {
          default: () => {
            var T;
            return [f("div", {
              class: Mt("panel")
            }, [(T = t.default) == null ? void 0 : T.call(t)])];
          }
        });
      const w = o.value || m || !I ? (d = t.default) == null ? void 0 : d.call(t) : null;
      return an(f("div", {
        id: n,
        role: "tabpanel",
        class: Mt("panel"),
        tabindex: x ? 0 : -1,
        "aria-labelledby": A
      }, [w]), [[ln, x]]);
    };
  }
});
const Ll = se(Ql), ql = se(pl), [Ho, Zt] = W("picker-group"), No = Symbol(Ho), Fl = ce({
  tabs: tt(),
  activeTab: le(0),
  nextStepText: String
}, Ht);
Q({
  name: Ho,
  props: Fl,
  emits: ["confirm", "cancel", "update:activeTab"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = kl(() => e.activeTab, (u) => t("update:activeTab", u)), {
      children: l,
      linkChildren: a
    } = it(No);
    a();
    const c = () => +o.value < e.tabs.length - 1 && e.nextStepText, r = () => {
      c() ? o.value = +o.value + 1 : t("confirm", l.map((u) => u.confirm()));
    }, i = () => t("cancel");
    return () => {
      var u;
      const s = (u = n.default) == null ? void 0 : u.call(n), g = c() ? e.nextStepText : e.confirmButtonText;
      return f("div", {
        class: Zt()
      }, [f(Io, {
        title: e.title,
        cancelButtonText: e.cancelButtonText,
        confirmButtonText: g,
        onConfirm: r,
        onCancel: i
      }, Ie(n, So)), f(ql, {
        active: o.value,
        "onUpdate:active": (d) => o.value = d,
        class: Zt("tabs"),
        shrink: !0,
        animated: !0,
        lazyRender: !1
      }, {
        default: () => [e.tabs.map((d, A) => f(Ll, {
          title: d,
          titleClass: Zt("tab-title")
        }, {
          default: () => [s == null ? void 0 : s[A]]
        }))]
      })]);
    };
  }
});
const po = ce({
  loading: Boolean,
  readonly: Boolean,
  allowHtml: Boolean,
  optionHeight: le(44),
  showToolbar: q,
  swipeDuration: le(1e3),
  visibleOptionNum: le(6)
}, Ht), Vl = ce({}, po, {
  columns: tt(),
  modelValue: tt(),
  toolbarPosition: V("top"),
  columnsFieldNames: Object
});
var Wl = Q({
  name: dl,
  props: Vl,
  emits: ["confirm", "cancel", "change", "scrollInto", "clickOption", "update:modelValue"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = E(), l = E(e.modelValue.slice(0)), {
      parent: a
    } = qe(No), {
      children: c,
      linkChildren: r
    } = it(Eo);
    r();
    const i = O(() => ml(e.columnsFieldNames)), u = O(() => fn(e.optionHeight)), s = O(() => fl(e.columns, i.value)), g = O(() => {
      const {
        columns: h
      } = e;
      switch (s.value) {
        case "multiple":
          return h;
        case "cascade":
          return vl(h, i.value, l);
        default:
          return [h];
      }
    }), d = O(() => g.value.some((h) => h.length)), A = O(() => g.value.map((h, P) => Wt(h, l.value[P], i.value))), y = O(() => g.value.map((h, P) => h.findIndex((z) => z[i.value.value] === l.value[P]))), S = (h, P) => {
      if (l.value[h] !== P) {
        const z = l.value.slice(0);
        z[h] = P, l.value = z;
      }
    }, m = () => ({
      selectedValues: l.value.slice(0),
      selectedOptions: A.value,
      selectedIndexes: y.value
    }), I = (h, P) => {
      S(P, h), s.value === "cascade" && l.value.forEach((z, U) => {
        const b = g.value[U];
        Bn(b, z, i.value) || S(U, b.length ? b[0][i.value.value] : void 0);
      }), te(() => {
        t("change", ce({
          columnIndex: P
        }, m()));
      });
    }, x = (h, P) => {
      const z = {
        columnIndex: P,
        currentOption: h
      };
      t("clickOption", ce(m(), z)), t("scrollInto", z);
    }, M = () => {
      c.forEach((P) => P.stopMomentum());
      const h = m();
      return te(() => {
        t("confirm", h);
      }), h;
    }, w = () => t("cancel", m()), T = () => g.value.map((h, P) => f(hl, {
      value: l.value[P],
      fields: i.value,
      options: h,
      readonly: e.readonly,
      allowHtml: e.allowHtml,
      optionHeight: u.value,
      swipeDuration: e.swipeDuration,
      visibleOptionNum: e.visibleOptionNum,
      onChange: (z) => I(z, P),
      onClickOption: (z) => x(z, P),
      onScrollInto: (z) => {
        t("scrollInto", {
          currentOption: z,
          columnIndex: P
        });
      }
    }, {
      option: n.option
    })), B = (h) => {
      if (d.value) {
        const P = {
          height: `${u.value}px`
        }, z = {
          backgroundSize: `100% ${(h - u.value) / 2}px`
        };
        return [f("div", {
          class: Ce("mask"),
          style: z
        }, null), f("div", {
          class: [Ca, Ce("frame")],
          style: P
        }, null)];
      }
    }, K = () => {
      const h = u.value * +e.visibleOptionNum, P = {
        height: `${h}px`
      };
      return f("div", {
        ref: o,
        class: Ce("columns"),
        style: P
      }, [T(), B(h)]);
    }, ne = () => {
      if (e.showToolbar && !a)
        return f(Io, fe(Ie(e, bl), {
          onConfirm: M,
          onCancel: w
        }), Ie(n, So));
    };
    Z(g, (h) => {
      h.forEach((P, z) => {
        P.length && !Bn(P, l.value[z], i.value) && S(z, Po(P)[i.value.value]);
      });
    }, {
      immediate: !0
    });
    let $;
    return Z(() => e.modelValue, (h) => {
      !pt(h, l.value) && !pt(h, $) && (l.value = h.slice(0), $ = h.slice(0));
    }, {
      deep: !0
    }), Z(l, (h) => {
      pt(h, e.modelValue) || ($ = h.slice(0), t("update:modelValue", $));
    }, {
      immediate: !0
    }), ze("touchmove", xe, {
      target: o
    }), ye({
      confirm: M,
      getSelectedOptions: () => A.value
    }), () => {
      var h, P;
      return f("div", {
        class: Ce()
      }, [e.toolbarPosition === "top" ? ne() : null, e.loading ? f(ut, {
        class: Ce("loading")
      }, null) : null, (h = n["columns-top"]) == null ? void 0 : h.call(n), K(), (P = n["columns-bottom"]) == null ? void 0 : P.call(n), e.toolbarPosition === "bottom" ? ne() : null]);
    };
  }
});
const Kl = se(Wl), [$l, Me] = W("cell"), Bo = {
  tag: V("div"),
  icon: String,
  size: String,
  title: Y,
  value: Y,
  label: Y,
  center: Boolean,
  isLink: Boolean,
  border: q,
  required: Boolean,
  iconPrefix: String,
  valueClass: je,
  labelClass: je,
  titleClass: je,
  titleStyle: null,
  arrowDirection: String,
  clickable: {
    type: Boolean,
    default: null
  }
}, _l = ce({}, Bo, mn);
var ei = Q({
  name: $l,
  props: _l,
  setup(e, {
    slots: t
  }) {
    const n = mo(), o = () => {
      if (t.label || ae(e.label))
        return f("div", {
          class: [Me("label"), e.labelClass]
        }, [t.label ? t.label() : e.label]);
    }, l = () => {
      var i;
      if (t.title || ae(e.title)) {
        const u = (i = t.title) == null ? void 0 : i.call(t);
        return Array.isArray(u) && u.length === 0 ? void 0 : f("div", {
          class: [Me("title"), e.titleClass],
          style: e.titleStyle
        }, [u || f("span", null, [e.title]), o()]);
      }
    }, a = () => {
      const i = t.value || t.default;
      if (i || ae(e.value))
        return f("div", {
          class: [Me("value"), e.valueClass]
        }, [i ? i() : f("span", null, [e.value])]);
    }, c = () => {
      if (t.icon)
        return t.icon();
      if (e.icon)
        return f(Te, {
          name: e.icon,
          class: Me("left-icon"),
          classPrefix: e.iconPrefix
        }, null);
    }, r = () => {
      if (t["right-icon"])
        return t["right-icon"]();
      if (e.isLink) {
        const i = e.arrowDirection && e.arrowDirection !== "right" ? `arrow-${e.arrowDirection}` : "arrow";
        return f(Te, {
          name: i,
          class: Me("right-icon")
        }, null);
      }
    };
    return () => {
      var i;
      const {
        tag: u,
        size: s,
        center: g,
        border: d,
        isLink: A,
        required: y
      } = e, S = (i = e.clickable) != null ? i : A, m = {
        center: g,
        required: y,
        clickable: S,
        borderless: !d
      };
      return s && (m[s] = !!s), f(u, {
        class: Me(m),
        role: S ? "button" : void 0,
        tabindex: S ? 0 : void 0,
        onClick: n
      }, {
        default: () => {
          var I;
          return [c(), l(), a(), r(), (I = t.extra) == null ? void 0 : I.call(t)];
        }
      });
    };
  }
});
const ti = se(ei);
function Ro(e) {
  return Array.isArray(e) ? !e.length : e === 0 ? !1 : !e;
}
function ni(e, t) {
  if (Ro(e)) {
    if (t.required)
      return !1;
    if (t.validateEmpty === !1)
      return !0;
  }
  return !(t.pattern && !t.pattern.test(String(e)));
}
function oi(e, t) {
  return new Promise((n) => {
    const o = t.validator(e, t);
    if (no(o)) {
      o.then(n);
      return;
    }
    n(o);
  });
}
function Yn(e, t) {
  const { message: n } = t;
  return It(n) ? n(e, t) : n || "";
}
function ai({ target: e }) {
  e.composing = !0;
}
function Xn({ target: e }) {
  e.composing && (e.composing = !1, e.dispatchEvent(new Event("input")));
}
function li(e, t) {
  const n = un();
  e.style.height = "auto";
  let o = e.scrollHeight;
  if (at(t)) {
    const { maxHeight: l, minHeight: a } = t;
    l !== void 0 && (o = Math.min(o, l)), a !== void 0 && (o = Math.max(o, a));
  }
  o && (e.style.height = `${o}px`, dn(n));
}
function ii(e) {
  return e === "number" ? {
    type: "text",
    inputmode: "decimal"
  } : e === "digit" ? {
    type: "tel",
    inputmode: "numeric"
  } : { type: e };
}
function we(e) {
  return [...e].length;
}
function Yt(e, t) {
  return [...e].slice(0, t).join("");
}
const [ri, be] = W("field"), ci = {
  id: String,
  name: String,
  leftIcon: String,
  rightIcon: String,
  autofocus: Boolean,
  clearable: Boolean,
  maxlength: Y,
  formatter: Function,
  clearIcon: V("clear"),
  modelValue: le(""),
  inputAlign: String,
  placeholder: String,
  autocomplete: String,
  errorMessage: String,
  enterkeyhint: String,
  clearTrigger: V("focus"),
  formatTrigger: V("onChange"),
  error: {
    type: Boolean,
    default: null
  },
  disabled: {
    type: Boolean,
    default: null
  },
  readonly: {
    type: Boolean,
    default: null
  }
}, si = ce({}, Bo, ci, {
  rows: Y,
  type: V("text"),
  rules: Array,
  autosize: [Boolean, Object],
  labelWidth: Y,
  labelClass: je,
  labelAlign: String,
  showWordLimit: Boolean,
  errorMessageAlign: String,
  colon: {
    type: Boolean,
    default: null
  }
});
var ui = Q({
  name: ri,
  props: si,
  emits: ["blur", "focus", "clear", "keypress", "clickInput", "endValidate", "startValidate", "clickLeftIcon", "clickRightIcon", "update:modelValue"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = yn(), l = De({
      status: "unvalidated",
      focused: !1,
      validateMessage: ""
    }), a = E(), c = E(), r = E(), {
      parent: i
    } = qe(ja), u = () => {
      var v;
      return String((v = e.modelValue) != null ? v : "");
    }, s = (v) => {
      if (ae(e[v]))
        return e[v];
      if (i && ae(i.props[v]))
        return i.props[v];
    }, g = O(() => {
      const v = s("readonly");
      if (e.clearable && !v) {
        const C = u() !== "", N = e.clearTrigger === "always" || e.clearTrigger === "focus" && l.focused;
        return C && N;
      }
      return !1;
    }), d = O(() => r.value && n.input ? r.value() : e.modelValue), A = (v) => v.reduce((C, N) => C.then(() => {
      if (l.status === "failed")
        return;
      let {
        value: J
      } = d;
      if (N.formatter && (J = N.formatter(J, N)), !ni(J, N)) {
        l.status = "failed", l.validateMessage = Yn(J, N);
        return;
      }
      if (N.validator)
        return Ro(J) && N.validateEmpty === !1 ? void 0 : oi(J, N).then((L) => {
          L && typeof L == "string" ? (l.status = "failed", l.validateMessage = L) : L === !1 && (l.status = "failed", l.validateMessage = Yn(J, N));
        });
    }), Promise.resolve()), y = () => {
      l.status = "unvalidated", l.validateMessage = "";
    }, S = () => t("endValidate", {
      status: l.status,
      message: l.validateMessage
    }), m = (v = e.rules) => new Promise((C) => {
      y(), v ? (t("startValidate"), A(v).then(() => {
        l.status === "failed" ? (C({
          name: e.name,
          message: l.validateMessage
        }), S()) : (l.status = "passed", C(), S());
      })) : C();
    }), I = (v) => {
      if (i && e.rules) {
        const {
          validateTrigger: C
        } = i.props, N = Sn(C).includes(v), J = e.rules.filter((L) => L.trigger ? Sn(L.trigger).includes(v) : N);
        J.length && m(J);
      }
    }, x = (v) => {
      var C;
      const {
        maxlength: N
      } = e;
      if (ae(N) && we(v) > +N) {
        const J = u();
        if (J && we(J) === +N)
          return J;
        const L = (C = a.value) == null ? void 0 : C.selectionEnd;
        if (l.focused && L) {
          const ee = [...v], ve = ee.length - +N;
          return ee.splice(L - ve, ve), ee.join("");
        }
        return Yt(v, +N);
      }
      return v;
    }, M = (v, C = "onChange") => {
      const N = v;
      v = x(v);
      const J = we(N) - we(v);
      if (e.type === "number" || e.type === "digit") {
        const ee = e.type === "number";
        v = Pa(v, ee, ee);
      }
      let L = 0;
      if (e.formatter && C === e.formatTrigger) {
        const {
          formatter: ee,
          maxlength: ve
        } = e;
        if (v = ee(v), ae(ve) && we(v) > +ve && (v = Yt(v, +ve)), a.value && l.focused) {
          const {
            selectionEnd: ft
          } = a.value, On = Yt(N, ft);
          L = we(ee(On)) - we(On);
        }
      }
      if (a.value && a.value.value !== v)
        if (l.focused) {
          let {
            selectionStart: ee,
            selectionEnd: ve
          } = a.value;
          if (a.value.value = v, ae(ee) && ae(ve)) {
            const ft = we(v);
            J ? (ee -= J, ve -= J) : L && (ee += L, ve += L), a.value.setSelectionRange(Math.min(ee, ft), Math.min(ve, ft));
          }
        } else
          a.value.value = v;
      v !== e.modelValue && t("update:modelValue", v);
    }, w = (v) => {
      v.target.composing || M(v.target.value);
    }, T = () => {
      var v;
      return (v = a.value) == null ? void 0 : v.blur();
    }, B = () => {
      var v;
      return (v = a.value) == null ? void 0 : v.focus();
    }, K = () => {
      const v = a.value;
      e.type === "textarea" && e.autosize && v && li(v, e.autosize);
    }, ne = (v) => {
      l.focused = !0, t("focus", v), te(K), s("readonly") && T();
    }, $ = (v) => {
      l.focused = !1, M(u(), "onBlur"), t("blur", v), !s("readonly") && (I("onBlur"), te(K), fa());
    }, k = (v) => t("clickInput", v), h = (v) => t("clickLeftIcon", v), P = (v) => t("clickRightIcon", v), z = (v) => {
      xe(v), t("update:modelValue", ""), t("clear", v);
    }, U = O(() => {
      if (typeof e.error == "boolean")
        return e.error;
      if (i && i.props.showError && l.status === "failed")
        return !0;
    }), b = O(() => {
      const v = s("labelWidth"), C = s("labelAlign");
      if (v && C !== "top")
        return {
          width: ge(v)
        };
    }), j = (v) => {
      v.keyCode === 13 && (!(i && i.props.submitOnEnter) && e.type !== "textarea" && xe(v), e.type === "search" && T()), t("keypress", v);
    }, R = () => e.id || `${o}-input`, G = () => l.status, ie = () => {
      const v = be("control", [s("inputAlign"), {
        error: U.value,
        custom: !!n.input,
        "min-height": e.type === "textarea" && !e.autosize
      }]);
      if (n.input)
        return f("div", {
          class: v,
          onClick: k
        }, [n.input()]);
      const C = {
        id: R(),
        ref: a,
        name: e.name,
        rows: e.rows !== void 0 ? +e.rows : void 0,
        class: v,
        disabled: s("disabled"),
        readonly: s("readonly"),
        autofocus: e.autofocus,
        placeholder: e.placeholder,
        autocomplete: e.autocomplete,
        enterkeyhint: e.enterkeyhint,
        "aria-labelledby": e.label ? `${o}-label` : void 0,
        onBlur: $,
        onFocus: ne,
        onInput: w,
        onClick: k,
        onChange: Xn,
        onKeypress: j,
        onCompositionend: Xn,
        onCompositionstart: ai
      };
      return e.type === "textarea" ? f("textarea", C, null) : f("input", fe(ii(e.type), C), null);
    }, _ = () => {
      const v = n["left-icon"];
      if (e.leftIcon || v)
        return f("div", {
          class: be("left-icon"),
          onClick: h
        }, [v ? v() : f(Te, {
          name: e.leftIcon,
          classPrefix: e.iconPrefix
        }, null)]);
    }, H = () => {
      const v = n["right-icon"];
      if (e.rightIcon || v)
        return f("div", {
          class: be("right-icon"),
          onClick: P
        }, [v ? v() : f(Te, {
          name: e.rightIcon,
          classPrefix: e.iconPrefix
        }, null)]);
    }, p = () => {
      if (e.showWordLimit && e.maxlength) {
        const v = we(u());
        return f("div", {
          class: be("word-limit")
        }, [f("span", {
          class: be("word-num")
        }, [v]), rn("/"), e.maxlength]);
      }
    }, X = () => {
      if (i && i.props.showErrorMessage === !1)
        return;
      const v = e.errorMessage || l.validateMessage;
      if (v) {
        const C = n["error-message"], N = s("errorMessageAlign");
        return f("div", {
          class: be("error-message", N)
        }, [C ? C({
          message: v
        }) : v]);
      }
    }, ue = () => {
      const v = s("labelWidth"), C = s("labelAlign"), N = s("colon") ? ":" : "";
      if (n.label)
        return [n.label(), N];
      if (e.label)
        return f("label", {
          id: `${o}-label`,
          for: R(),
          onClick: (J) => {
            xe(J), B();
          },
          style: C === "top" && v ? {
            width: ge(v)
          } : void 0
        }, [e.label + N]);
    }, D = () => [f("div", {
      class: be("body")
    }, [ie(), g.value && f(Te, {
      ref: c,
      name: e.clearIcon,
      class: be("clear")
    }, null), H(), n.button && f("div", {
      class: be("button")
    }, [n.button()])]), p(), X()];
    return ye({
      blur: T,
      focus: B,
      validate: m,
      formValue: d,
      resetValidation: y,
      getValidationStatus: G
    }), ot(ua, {
      customValue: r,
      resetValidation: y,
      validateWithTrigger: I
    }), Z(() => e.modelValue, () => {
      M(u()), y(), I("onChange"), te(K);
    }), Ee(() => {
      M(u(), e.formatTrigger), te(K);
    }), ze("touchstart", z, {
      target: O(() => {
        var v;
        return (v = c.value) == null ? void 0 : v.$el;
      })
    }), () => {
      const v = s("disabled"), C = s("labelAlign"), N = _(), J = () => {
        const L = ue();
        return C === "top" ? [N, L].filter(Boolean) : L || [];
      };
      return f(ti, {
        size: e.size,
        class: be({
          error: U.value,
          disabled: v,
          [`label-${C}`]: C
        }),
        center: e.center,
        border: e.border,
        isLink: e.isLink,
        clickable: e.clickable,
        titleStyle: b.value,
        valueClass: be("value"),
        titleClass: [be("label", [C, {
          required: e.required
        }]), e.labelClass],
        arrowDirection: e.arrowDirection
      }, {
        icon: N && C !== "top" ? () => N : null,
        title: J,
        value: D,
        extra: n.extra
      });
    };
  }
});
const di = se(ui);
let Ve = 0;
function fi(e) {
  e ? (Ve || document.body.classList.add("van-toast--unclickable"), Ve++) : Ve && (Ve--, Ve || document.body.classList.remove("van-toast--unclickable"));
}
const [vi, Ze] = W("toast"), gi = ["show", "overlay", "teleport", "transition", "overlayClass", "overlayStyle", "closeOnClickOverlay"], mi = {
  icon: String,
  show: Boolean,
  type: V("text"),
  overlay: Boolean,
  message: Y,
  iconSize: Y,
  duration: ao(2e3),
  position: V("middle"),
  teleport: [String, Object],
  wordBreak: String,
  className: je,
  iconPrefix: String,
  transition: V("van-fade"),
  loadingType: String,
  forbidClick: Boolean,
  overlayClass: je,
  overlayStyle: Object,
  closeOnClick: Boolean,
  closeOnClickOverlay: Boolean
};
var Jo = Q({
  name: vi,
  props: mi,
  emits: ["update:show"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    let o, l = !1;
    const a = () => {
      const g = e.show && e.forbidClick;
      l !== g && (l = g, fi(l));
    }, c = (g) => t("update:show", g), r = () => {
      e.closeOnClick && c(!1);
    }, i = () => clearTimeout(o), u = () => {
      const {
        icon: g,
        type: d,
        iconSize: A,
        iconPrefix: y,
        loadingType: S
      } = e;
      if (g || d === "success" || d === "fail")
        return f(Te, {
          name: g || d,
          size: A,
          class: Ze("icon"),
          classPrefix: y
        }, null);
      if (d === "loading")
        return f(ut, {
          class: Ze("loading"),
          size: A,
          type: S
        }, null);
    }, s = () => {
      const {
        type: g,
        message: d
      } = e;
      if (n.message)
        return f("div", {
          class: Ze("text")
        }, [n.message()]);
      if (ae(d) && d !== "")
        return g === "html" ? f("div", {
          key: 0,
          class: Ze("text"),
          innerHTML: String(d)
        }, null) : f("div", {
          class: Ze("text")
        }, [d]);
    };
    return Z(() => [e.show, e.forbidClick], a), Z(() => [e.show, e.type, e.message, e.duration], () => {
      i(), e.show && e.duration > 0 && (o = setTimeout(() => {
        c(!1);
      }, e.duration));
    }), Ee(a), on(a), () => f(hn, fe({
      class: [Ze([e.position, e.wordBreak === "normal" ? "break-normal" : e.wordBreak, {
        [e.type]: !e.icon
      }]), e.className],
      lockScroll: !1,
      onClick: r,
      onClosed: i,
      "onUpdate:show": c
    }, Ie(e, gi)), {
      default: () => [u(), s()]
    });
  }
});
function Ai() {
  const e = De({
    show: !1
  }), t = (l) => {
    e.show = l;
  }, n = (l) => {
    ce(e, l, { transitionAppear: !0 }), t(!0);
  }, o = () => t(!1);
  return ye({ open: n, close: o, toggle: t }), {
    open: n,
    close: o,
    state: e,
    toggle: t
  };
}
function hi(e) {
  const t = qo(e), n = document.createElement("div");
  return document.body.appendChild(n), {
    instance: t.mount(n),
    unmount() {
      t.unmount(), document.body.removeChild(n);
    }
  };
}
const yi = {
  icon: "",
  type: "text",
  message: "",
  className: "",
  overlay: !1,
  onClose: void 0,
  onOpened: void 0,
  duration: 2e3,
  teleport: "body",
  iconSize: void 0,
  iconPrefix: void 0,
  position: "middle",
  transition: "van-fade",
  forbidClick: !1,
  loadingType: void 0,
  overlayClass: "",
  overlayStyle: void 0,
  closeOnClick: !1,
  closeOnClickOverlay: !1
};
let ht = [], bi = !1, Un = ce({}, yi);
const ki = /* @__PURE__ */ new Map();
function Pi(e) {
  return at(e) ? e : {
    message: e
  };
}
function Oi() {
  const {
    instance: e,
    unmount: t
  } = hi({
    setup() {
      const n = E(""), {
        open: o,
        state: l,
        close: a,
        toggle: c
      } = Ai(), r = () => {
      }, i = () => f(Jo, fe(l, {
        onClosed: r,
        "onUpdate:show": c
      }), null);
      return Z(n, (u) => {
        l.message = u;
      }), Re().render = i, {
        open: o,
        close: a,
        message: n
      };
    }
  });
  return e;
}
function Ei() {
  if (!ht.length || bi) {
    const e = Oi();
    ht.push(e);
  }
  return ht[ht.length - 1];
}
function Gn(e = {}) {
  if (!lt)
    return {};
  const t = Ei(), n = Pi(e);
  return t.open(ce({}, Un, ki.get(n.type || Un.type), n)), t;
}
se(Jo);
const [Si, de, He] = W("calendar"), Ii = (e) => He("monthTitle", e.getFullYear(), e.getMonth() + 1);
function Kt(e, t) {
  const n = e.getFullYear(), o = t.getFullYear();
  if (n === o) {
    const l = e.getMonth(), a = t.getMonth();
    return l === a ? 0 : l > a ? 1 : -1;
  }
  return n > o ? 1 : -1;
}
function Ae(e, t) {
  const n = Kt(e, t);
  if (n === 0) {
    const o = e.getDate(), l = t.getDate();
    return o === l ? 0 : o > l ? 1 : -1;
  }
  return n;
}
const Tt = (e) => new Date(e), Qn = (e) => Array.isArray(e) ? e.map(Tt) : Tt(e);
function bn(e, t) {
  const n = Tt(e);
  return n.setDate(n.getDate() + t), n;
}
const $t = (e) => bn(e, -1), Mo = (e) => bn(e, 1), _t = () => {
  const e = /* @__PURE__ */ new Date();
  return e.setHours(0, 0, 0, 0), e;
};
function xi(e) {
  const t = e[0].getTime();
  return (e[1].getTime() - t) / (1e3 * 60 * 60 * 24) + 1;
}
ce({}, po, {
  modelValue: tt(),
  filter: Function,
  formatter: {
    type: Function,
    default: (e, t) => t
  }
});
const Ti = (e, t) => 32 - new Date(e, t - 1, 32).getDate(), [Di] = W("calendar-day");
var zi = Q({
  name: Di,
  props: {
    item: Oe(Object),
    color: String,
    index: Number,
    offset: ao(0),
    rowHeight: String
  },
  emits: ["click"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = O(() => {
      var i;
      const {
        item: u,
        index: s,
        color: g,
        offset: d,
        rowHeight: A
      } = e, y = {
        height: A
      };
      if (u.type === "placeholder")
        return y.width = "100%", y;
      if (s === 0 && (y.marginLeft = `${100 * d / 7}%`), g)
        switch (u.type) {
          case "end":
          case "start":
          case "start-end":
          case "multiple-middle":
          case "multiple-selected":
            y.background = g;
            break;
          case "middle":
            y.color = g;
            break;
        }
      return d + (((i = u.date) == null ? void 0 : i.getDate()) || 1) > 28 && (y.marginBottom = 0), y;
    }), l = () => {
      e.item.type !== "disabled" && t("click", e.item);
    }, a = () => {
      const {
        topInfo: i
      } = e.item;
      if (i || n["top-info"])
        return f("div", {
          class: de("top-info")
        }, [n["top-info"] ? n["top-info"](e.item) : i]);
    }, c = () => {
      const {
        bottomInfo: i
      } = e.item;
      if (i || n["bottom-info"])
        return f("div", {
          class: de("bottom-info")
        }, [n["bottom-info"] ? n["bottom-info"](e.item) : i]);
    }, r = () => {
      const {
        item: i,
        color: u,
        rowHeight: s
      } = e, {
        type: g,
        text: d
      } = i, A = [a(), d, c()];
      return g === "selected" ? f("div", {
        class: de("selected-day"),
        style: {
          width: s,
          height: s,
          background: u
        }
      }, [A]) : A;
    };
    return () => {
      const {
        type: i,
        className: u
      } = e.item;
      return i === "placeholder" ? f("div", {
        class: de("day"),
        style: o.value
      }, null) : f("div", {
        role: "gridcell",
        style: o.value,
        class: [de("day", i), u],
        tabindex: i === "disabled" ? void 0 : -1,
        onClick: l
      }, [r()]);
    };
  }
});
const [wi] = W("calendar-month"), Ci = {
  date: Oe(Date),
  type: String,
  color: String,
  minDate: Oe(Date),
  maxDate: Oe(Date),
  showMark: Boolean,
  rowHeight: Y,
  formatter: Function,
  lazyRender: Boolean,
  currentDate: [Date, Array],
  allowSameDay: Boolean,
  showSubtitle: Boolean,
  showMonthTitle: Boolean,
  firstDayOfWeek: Number
};
var ji = Q({
  name: wi,
  props: Ci,
  emits: ["click"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const [o, l] = oa(), a = E(), c = E(), r = Ha(c), i = O(() => Ii(e.date)), u = O(() => ge(e.rowHeight)), s = O(() => {
      const k = e.date.getDay();
      return e.firstDayOfWeek ? (k + 7 - e.firstDayOfWeek) % 7 : k;
    }), g = O(() => Ti(e.date.getFullYear(), e.date.getMonth() + 1)), d = O(() => o.value || !e.lazyRender), A = () => i.value, y = (k) => {
      const h = (P) => e.currentDate.some((z) => Ae(z, P) === 0);
      if (h(k)) {
        const P = $t(k), z = Mo(k), U = h(P), b = h(z);
        return U && b ? "multiple-middle" : U ? "end" : b ? "start" : "multiple-selected";
      }
      return "";
    }, S = (k) => {
      const [h, P] = e.currentDate;
      if (!h)
        return "";
      const z = Ae(k, h);
      if (!P)
        return z === 0 ? "start" : "";
      const U = Ae(k, P);
      return e.allowSameDay && z === 0 && U === 0 ? "start-end" : z === 0 ? "start" : U === 0 ? "end" : z > 0 && U < 0 ? "middle" : "";
    }, m = (k) => {
      const {
        type: h,
        minDate: P,
        maxDate: z,
        currentDate: U
      } = e;
      if (Ae(k, P) < 0 || Ae(k, z) > 0)
        return "disabled";
      if (U === null)
        return "";
      if (Array.isArray(U)) {
        if (h === "multiple")
          return y(k);
        if (h === "range")
          return S(k);
      } else if (h === "single")
        return Ae(k, U) === 0 ? "selected" : "";
      return "";
    }, I = (k) => {
      if (e.type === "range") {
        if (k === "start" || k === "end")
          return He(k);
        if (k === "start-end")
          return `${He("start")}/${He("end")}`;
      }
    }, x = () => {
      if (e.showMonthTitle)
        return f("div", {
          class: de("month-title")
        }, [n["month-title"] ? n["month-title"]({
          date: e.date,
          text: i.value
        }) : i.value]);
    }, M = () => {
      if (e.showMark && d.value)
        return f("div", {
          class: de("month-mark")
        }, [e.date.getMonth() + 1]);
    }, w = O(() => {
      const k = Math.ceil((g.value + s.value) / 7);
      return Array(k).fill({
        type: "placeholder"
      });
    }), T = O(() => {
      const k = [], h = e.date.getFullYear(), P = e.date.getMonth();
      for (let z = 1; z <= g.value; z++) {
        const U = new Date(h, P, z), b = m(U);
        let j = {
          date: U,
          type: b,
          text: z,
          bottomInfo: I(b)
        };
        e.formatter && (j = e.formatter(j)), k.push(j);
      }
      return k;
    }), B = O(() => T.value.filter((k) => k.type === "disabled")), K = (k, h) => {
      if (a.value) {
        const P = he(a.value), z = w.value.length, b = (Math.ceil((h.getDate() + s.value) / 7) - 1) * P.height / z;
        xt(k, P.top + b + k.scrollTop - he(k).top);
      }
    }, ne = (k, h) => f(zi, {
      item: k,
      index: h,
      color: e.color,
      offset: s.value,
      rowHeight: u.value,
      onClick: (P) => t("click", P)
    }, Ie(n, ["top-info", "bottom-info"])), $ = () => f("div", {
      ref: a,
      role: "grid",
      class: de("days")
    }, [M(), (d.value ? T : w).value.map(ne)]);
    return ye({
      getTitle: A,
      getHeight: () => r.value,
      setVisible: l,
      scrollToDate: K,
      disabledDays: B
    }), () => f("div", {
      class: de("month"),
      ref: c
    }, [x(), $()]);
  }
});
const [Hi] = W("calendar-header");
var Ni = Q({
  name: Hi,
  props: {
    date: Date,
    title: String,
    subtitle: String,
    showTitle: Boolean,
    showSubtitle: Boolean,
    firstDayOfWeek: Number
  },
  emits: ["clickSubtitle"],
  setup(e, {
    slots: t,
    emit: n
  }) {
    const o = () => {
      if (e.showTitle) {
        const r = e.title || He("title"), i = t.title ? t.title() : r;
        return f("div", {
          class: de("header-title")
        }, [i]);
      }
    }, l = (r) => n("clickSubtitle", r), a = () => {
      if (e.showSubtitle) {
        const r = t.subtitle ? t.subtitle({
          date: e.date,
          text: e.subtitle
        }) : e.subtitle;
        return f("div", {
          class: de("header-subtitle"),
          onClick: l
        }, [r]);
      }
    }, c = () => {
      const {
        firstDayOfWeek: r
      } = e, i = He("weekdays"), u = [...i.slice(r, 7), ...i.slice(0, r)];
      return f("div", {
        class: de("weekdays")
      }, [u.map((s) => f("span", {
        class: de("weekday")
      }, [s]))]);
    };
    return () => f("div", {
      class: de("header")
    }, [o(), a(), c()]);
  }
});
const pi = {
  show: Boolean,
  type: V("single"),
  title: String,
  color: String,
  round: q,
  readonly: Boolean,
  poppable: q,
  maxRange: le(null),
  position: V("bottom"),
  teleport: [String, Object],
  showMark: q,
  showTitle: q,
  formatter: Function,
  rowHeight: Y,
  confirmText: String,
  rangePrompt: String,
  lazyRender: q,
  showConfirm: q,
  defaultDate: [Date, Array],
  allowSameDay: Boolean,
  showSubtitle: q,
  closeOnPopstate: q,
  showRangePrompt: q,
  confirmDisabledText: String,
  closeOnClickOverlay: q,
  safeAreaInsetTop: Boolean,
  safeAreaInsetBottom: q,
  minDate: {
    type: Date,
    validator: Lt,
    default: _t
  },
  maxDate: {
    type: Date,
    validator: Lt,
    default: () => {
      const e = _t();
      return new Date(e.getFullYear(), e.getMonth() + 6, e.getDate());
    }
  },
  firstDayOfWeek: {
    type: Y,
    default: 0,
    validator: (e) => e >= 0 && e <= 6
  }
};
var Bi = Q({
  name: Si,
  props: pi,
  emits: ["select", "confirm", "unselect", "monthShow", "overRange", "update:show", "clickSubtitle"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = (b, j = e.minDate, R = e.maxDate) => Ae(b, j) === -1 ? j : Ae(b, R) === 1 ? R : b, l = (b = e.defaultDate) => {
      const {
        type: j,
        minDate: R,
        maxDate: G,
        allowSameDay: ie
      } = e;
      if (b === null)
        return b;
      const _ = _t();
      if (j === "range") {
        Array.isArray(b) || (b = []);
        const H = o(b[0] || _, R, ie ? G : $t(G)), p = o(b[1] || _, ie ? R : Mo(R));
        return [H, p];
      }
      return j === "multiple" ? Array.isArray(b) ? b.map((H) => o(H)) : [o(_)] : ((!b || Array.isArray(b)) && (b = _), o(b));
    };
    let a;
    const c = E(), r = E({
      text: "",
      date: void 0
    }), i = E(l()), [u, s] = xo(), g = O(() => e.firstDayOfWeek ? +e.firstDayOfWeek % 7 : 0), d = O(() => {
      const b = [], j = new Date(e.minDate);
      j.setDate(1);
      do
        b.push(new Date(j)), j.setMonth(j.getMonth() + 1);
      while (Kt(j, e.maxDate) !== 1);
      return b;
    }), A = O(() => {
      if (i.value) {
        if (e.type === "range")
          return !i.value[0] || !i.value[1];
        if (e.type === "multiple")
          return !i.value.length;
      }
      return !i.value;
    }), y = () => i.value, S = () => {
      const b = ct(c.value), j = b + a, R = d.value.map((p, X) => u.value[X].getHeight()), G = R.reduce((p, X) => p + X, 0);
      if (j > G && b > 0)
        return;
      let ie = 0, _;
      const H = [-1, -1];
      for (let p = 0; p < d.value.length; p++) {
        const X = u.value[p];
        ie <= j && ie + R[p] >= b && (H[1] = p, _ || (_ = X, H[0] = p), u.value[p].showed || (u.value[p].showed = !0, t("monthShow", {
          date: X.date,
          title: X.getTitle()
        }))), ie += R[p];
      }
      d.value.forEach((p, X) => {
        const ue = X >= H[0] - 1 && X <= H[1] + 1;
        u.value[X].setVisible(ue);
      }), _ && (r.value = {
        text: _.getTitle(),
        date: _.date
      });
    }, m = (b) => {
      Be(() => {
        d.value.some((j, R) => Kt(j, b) === 0 ? (c.value && u.value[R].scrollToDate(c.value, b), !0) : !1), S();
      });
    }, I = () => {
      if (!(e.poppable && !e.show))
        if (i.value) {
          const b = e.type === "single" ? i.value : i.value[0];
          Lt(b) && m(b);
        } else
          Be(S);
    }, x = () => {
      e.poppable && !e.show || (Be(() => {
        a = Math.floor(he(c).height);
      }), I());
    }, M = (b = l()) => {
      i.value = b, I();
    }, w = (b) => {
      const {
        maxRange: j,
        rangePrompt: R,
        showRangePrompt: G
      } = e;
      return j && xi(b) > +j ? (G && Gn(R || He("rangePrompt", j)), t("overRange"), !1) : !0;
    }, T = () => {
      var b;
      return t("confirm", (b = i.value) != null ? b : Qn(i.value));
    }, B = (b, j) => {
      const R = (G) => {
        i.value = G, t("select", Qn(G));
      };
      if (j && e.type === "range" && !w(b)) {
        R([b[0], bn(b[0], +e.maxRange - 1)]);
        return;
      }
      R(b), j && !e.showConfirm && T();
    }, K = (b, j, R) => {
      var G;
      return (G = b.find((ie) => Ae(j, ie.date) === -1 && Ae(ie.date, R) === -1)) == null ? void 0 : G.date;
    }, ne = O(() => u.value.reduce((b, j) => {
      var R, G;
      return b.push(...(G = (R = j.disabledDays) == null ? void 0 : R.value) != null ? G : []), b;
    }, [])), $ = (b) => {
      if (e.readonly || !b.date)
        return;
      const {
        date: j
      } = b, {
        type: R
      } = e;
      if (R === "range") {
        if (!i.value) {
          B([j]);
          return;
        }
        const [G, ie] = i.value;
        if (G && !ie) {
          const _ = Ae(j, G);
          if (_ === 1) {
            const H = K(ne.value, G, j);
            if (H) {
              const p = $t(H);
              Ae(G, p) === -1 ? B([G, p]) : B([j]);
            } else
              B([G, j], !0);
          } else
            _ === -1 ? B([j]) : e.allowSameDay && B([j, j], !0);
        } else
          B([j]);
      } else if (R === "multiple") {
        if (!i.value) {
          B([j]);
          return;
        }
        const G = i.value, ie = G.findIndex((_) => Ae(_, j) === 0);
        if (ie !== -1) {
          const [_] = G.splice(ie, 1);
          t("unselect", Tt(_));
        } else
          e.maxRange && G.length >= +e.maxRange ? Gn(e.rangePrompt || He("rangePrompt", e.maxRange)) : B([...G, j]);
      } else
        B(j, !0);
    }, k = (b) => t("update:show", b), h = (b, j) => {
      const R = j !== 0 || !e.showSubtitle;
      return f(ji, fe({
        ref: s(j),
        date: b,
        currentDate: i.value,
        showMonthTitle: R,
        firstDayOfWeek: g.value
      }, Ie(e, ["type", "color", "minDate", "maxDate", "showMark", "formatter", "rowHeight", "lazyRender", "showSubtitle", "allowSameDay"]), {
        onClick: $
      }), Ie(n, ["top-info", "bottom-info", "month-title"]));
    }, P = () => {
      if (n.footer)
        return n.footer();
      if (e.showConfirm) {
        const b = n["confirm-text"], j = A.value, R = j ? e.confirmDisabledText : e.confirmText;
        return f(An, {
          round: !0,
          block: !0,
          type: "primary",
          color: e.color,
          class: de("confirm"),
          disabled: j,
          nativeType: "button",
          onClick: T
        }, {
          default: () => [b ? b({
            disabled: j
          }) : R || He("confirm")]
        });
      }
    }, z = () => f("div", {
      class: [de("footer"), {
        "van-safe-area-bottom": e.safeAreaInsetBottom
      }]
    }, [P()]), U = () => f("div", {
      class: de()
    }, [f(Ni, {
      date: r.value.date,
      title: e.title,
      subtitle: r.value.text,
      showTitle: e.showTitle,
      showSubtitle: e.showSubtitle,
      firstDayOfWeek: g.value,
      onClickSubtitle: (b) => t("clickSubtitle", b)
    }, Ie(n, ["title", "subtitle"])), f("div", {
      ref: c,
      class: de("body"),
      onScroll: S
    }, [d.value.map(h)]), z()]);
    return Z(() => e.show, x), Z(() => [e.type, e.minDate, e.maxDate], () => M(l(i.value))), Z(() => e.defaultDate, (b = null) => {
      i.value = b, I();
    }), ye({
      reset: M,
      scrollToDate: m,
      getSelectedDate: y
    }), rt(x), () => e.poppable ? f(hn, {
      show: e.show,
      class: de("popup"),
      round: e.round,
      position: e.position,
      closeable: e.showTitle || e.showSubtitle,
      teleport: e.teleport,
      closeOnPopstate: e.closeOnPopstate,
      safeAreaInsetTop: e.safeAreaInsetTop,
      closeOnClickOverlay: e.closeOnClickOverlay,
      "onUpdate:show": k
    }, {
      default: U
    }) : U();
  }
});
const Ri = se(Bi), [Zo, Ji] = W("row"), Yo = Symbol(Zo), Mi = {
  tag: V("div"),
  wrap: q,
  align: String,
  gutter: le(0),
  justify: String
};
var Zi = Q({
  name: Zo,
  props: Mi,
  setup(e, {
    slots: t
  }) {
    const {
      children: n,
      linkChildren: o
    } = it(Yo), l = O(() => {
      const c = [[]];
      let r = 0;
      return n.forEach((i, u) => {
        r += Number(i.span), r > 24 ? (c.push([u]), r -= 24) : c[c.length - 1].push(u);
      }), c;
    }), a = O(() => {
      const c = Number(e.gutter), r = [];
      return c && l.value.forEach((i) => {
        const u = c * (i.length - 1) / i.length;
        i.forEach((s, g) => {
          if (g === 0)
            r.push({
              right: u
            });
          else {
            const d = c - r[s - 1].right, A = u - d;
            r.push({
              left: d,
              right: A
            });
          }
        });
      }), r;
    });
    return o({
      spaces: a
    }), () => {
      const {
        tag: c,
        wrap: r,
        align: i,
        justify: u
      } = e;
      return f(c, {
        class: Ji({
          [`align-${i}`]: i,
          [`justify-${u}`]: u,
          nowrap: !r
        })
      }, {
        default: () => {
          var s;
          return [(s = t.default) == null ? void 0 : s.call(t)];
        }
      });
    };
  }
});
const [Yi, Xi] = W("col"), Ui = {
  tag: V("div"),
  span: le(0),
  offset: Y
};
var Gi = Q({
  name: Yi,
  props: Ui,
  setup(e, {
    slots: t
  }) {
    const {
      parent: n,
      index: o
    } = qe(Yo), l = O(() => {
      if (!n)
        return;
      const {
        spaces: a
      } = n;
      if (a && a.value && a.value[o.value]) {
        const {
          left: c,
          right: r
        } = a.value[o.value];
        return {
          paddingLeft: c ? `${c}px` : null,
          paddingRight: r ? `${r}px` : null
        };
      }
    });
    return () => {
      const {
        tag: a,
        span: c,
        offset: r
      } = e;
      return f(a, {
        style: l.value,
        class: Xi({
          [c]: c,
          [`offset-${r}`]: r
        })
      }, {
        default: () => {
          var i;
          return [(i = t.default) == null ? void 0 : i.call(t)];
        }
      });
    };
  }
});
const Xt = se(Gi), [Qi, Ye, Li] = W("list"), qi = {
  error: Boolean,
  offset: le(300),
  loading: Boolean,
  disabled: Boolean,
  finished: Boolean,
  errorText: String,
  direction: V("down"),
  loadingText: String,
  finishedText: String,
  immediateCheck: q
};
var Fi = Q({
  name: Qi,
  props: qi,
  emits: ["load", "update:error", "update:loading"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    const o = E(e.loading), l = E(), a = E(), c = Bl(), r = wt(l), i = () => {
      te(() => {
        if (o.value || e.finished || e.disabled || e.error || // skip check when inside an inactive tab
        (c == null ? void 0 : c.value) === !1)
          return;
        const {
          direction: A
        } = e, y = +e.offset, S = he(r);
        if (!S.height || Ge(l))
          return;
        let m = !1;
        const I = he(a);
        A === "up" ? m = S.top - I.top <= y : m = I.bottom - S.bottom <= y, m && (o.value = !0, t("update:loading", !0), t("load"));
      });
    }, u = () => {
      if (e.finished) {
        const A = n.finished ? n.finished() : e.finishedText;
        if (A)
          return f("div", {
            class: Ye("finished-text")
          }, [A]);
      }
    }, s = () => {
      t("update:error", !1), i();
    }, g = () => {
      if (e.error) {
        const A = n.error ? n.error() : e.errorText;
        if (A)
          return f("div", {
            role: "button",
            class: Ye("error-text"),
            tabindex: 0,
            onClick: s
          }, [A]);
      }
    }, d = () => {
      if (o.value && !e.finished && !e.disabled)
        return f("div", {
          class: Ye("loading")
        }, [n.loading ? n.loading() : f(ut, {
          class: Ye("loading-icon")
        }, {
          default: () => [e.loadingText || Li("loading")]
        })]);
    };
    return Z(() => [e.loading, e.finished, e.error], i), c && Z(c, (A) => {
      A && i();
    }), Fo(() => {
      o.value = e.loading;
    }), Ee(() => {
      e.immediateCheck && i();
    }), ye({
      check: i
    }), ze("scroll", i, {
      target: r,
      passive: !0
    }), () => {
      var A;
      const y = (A = n.default) == null ? void 0 : A.call(n), S = f("div", {
        ref: a,
        class: Ye("placeholder")
      }, null);
      return f("div", {
        ref: l,
        role: "feed",
        class: Ye(),
        "aria-busy": o.value
      }, [e.direction === "down" ? y : S, d(), u(), g(), e.direction === "up" ? y : S]);
    };
  }
});
const Vi = se(Fi), [Wi, We, Ki] = W("pull-refresh"), Xo = 50, $i = ["pulling", "loosing", "success"], _i = {
  disabled: Boolean,
  modelValue: Boolean,
  headHeight: le(Xo),
  successText: String,
  pullingText: String,
  loosingText: String,
  loadingText: String,
  pullDistance: Y,
  successDuration: le(500),
  animationDuration: le(300)
};
var er = Q({
  name: Wi,
  props: _i,
  emits: ["change", "refresh", "update:modelValue"],
  setup(e, {
    emit: t,
    slots: n
  }) {
    let o;
    const l = E(), a = E(), c = wt(l), r = De({
      status: "normal",
      distance: 0,
      duration: 0
    }), i = jt(), u = () => {
      if (e.headHeight !== Xo)
        return {
          height: `${e.headHeight}px`
        };
    }, s = () => r.status !== "loading" && r.status !== "success" && !e.disabled, g = (w) => {
      const T = +(e.pullDistance || e.headHeight);
      return w > T && (w < T * 2 ? w = T + (w - T) / 2 : w = T * 1.5 + (w - T * 2) / 4), Math.round(w);
    }, d = (w, T) => {
      const B = +(e.pullDistance || e.headHeight);
      r.distance = w, T ? r.status = "loading" : w === 0 ? r.status = "normal" : w < B ? r.status = "pulling" : r.status = "loosing", t("change", {
        status: r.status,
        distance: w
      });
    }, A = () => {
      const {
        status: w
      } = r;
      return w === "normal" ? "" : e[`${w}Text`] || Ki(w);
    }, y = () => {
      const {
        status: w,
        distance: T
      } = r;
      if (n[w])
        return n[w]({
          distance: T
        });
      const B = [];
      return $i.includes(w) && B.push(f("div", {
        class: We("text")
      }, [A()])), w === "loading" && B.push(f(ut, {
        class: We("loading")
      }, {
        default: A
      })), B;
    }, S = () => {
      r.status = "success", setTimeout(() => {
        d(0);
      }, +e.successDuration);
    }, m = (w) => {
      o = ct(c.value) === 0, o && (r.duration = 0, i.start(w));
    }, I = (w) => {
      s() && m(w);
    }, x = (w) => {
      if (s()) {
        o || m(w);
        const {
          deltaY: T
        } = i;
        i.move(w), o && T.value >= 0 && i.isVertical() && (xe(w), d(g(T.value)));
      }
    }, M = () => {
      o && i.deltaY.value && s() && (r.duration = +e.animationDuration, r.status === "loosing" ? (d(+e.headHeight, !0), t("update:modelValue", !0), te(() => t("refresh"))) : d(0));
    };
    return Z(() => e.modelValue, (w) => {
      r.duration = +e.animationDuration, w ? d(+e.headHeight, !0) : n.success || e.successText ? S() : d(0, !1);
    }), ze("touchmove", x, {
      target: a
    }), () => {
      var w;
      const T = {
        transitionDuration: `${r.duration}ms`,
        transform: r.distance ? `translate3d(0,${r.distance}px, 0)` : ""
      };
      return f("div", {
        ref: l,
        class: We()
      }, [f("div", {
        ref: a,
        class: We("track"),
        style: T,
        onTouchstartPassive: I,
        onTouchend: M,
        onTouchcancel: M
      }, [f("div", {
        class: We("head"),
        style: u()
      }, [y()]), (w = n.default) == null ? void 0 : w.call(n)])]);
    };
  }
});
const tr = se(er), nr = se(Zi);
const dt = (e, t) => {
  const n = e.__vccOpts || e;
  for (const [o, l] of t)
    n[o] = l;
  return n;
}, kn = Q({
  name: "KvButton",
  components: { VanButton: An },
  props: {
    type: {
      type: String,
      default: "default",
      validate: (e) => ["primary", "success", "warning", "danger", "text"].includes(e)
    },
    link: {
      type: Boolean,
      default: !1
    }
  },
  setup(e) {
    return { textcolor: O(() => `var(--van-button-${e.type}-background)`) };
  }
}), Ln = () => {
  to((e) => ({
    "094aed15": e.textcolor
  }));
}, qn = kn.setup;
kn.setup = qn ? (e, t) => (Ln(), qn(e, t)) : Ln;
function or(e, t, n, o, l, a) {
  const c = St("van-button");
  return F(), Se(c, fe({
    type: e.type,
    class: { "is-link": e.link }
  }, e.$attrs), Vo({
    loading: Pe(() => [
      ke(e.$slots, "loading", {}, void 0, !0)
    ]),
    default: Pe(() => [
      ke(e.$slots, "default", {}, void 0, !0)
    ]),
    _: 2
  }, [
    e.$attrs.icon ? void 0 : {
      name: "icon",
      fn: Pe(() => [
        ke(e.$slots, "icon", {}, void 0, !0)
      ]),
      key: "0"
    }
  ]), 1040, ["type", "class"]);
}
const yt = /* @__PURE__ */ dt(kn, [["render", or], ["__scopeId", "data-v-36fadbb1"]]);
yt.install = function(e) {
  e.component(yt.name, yt);
};
const ar = Q({
  name: "KvInput",
  components: { VanField: di },
  /**
    * @param1 { ?number } point 小数点后几位 :point="2"
    * @param2 { ?string } type 输入框的类型 digit 只能输入整数
    * @param3 { ?string } prefix 内容前 显示标识
    * @param4 { ?string } suffix 内容后 显示标识
    * @param5 { ?number } min 最小值
    * @param5 { ?number } max 最大值
    */
  props: {
    modelValue: { type: [Number, String], default: "" },
    point: { type: Number, default: 2 },
    type: { type: String, default: "number" },
    prefix: { type: String, default: "" },
    suffix: { type: String, default: "" },
    min: { type: Number, default: Number.NEGATIVE_INFINITY },
    max: { type: Number, default: Number.POSITIVE_INFINITY }
  },
  emits: ["update:modelValue", "clear"],
  setup(e, { emit: t }) {
    return {
      inputValue: O({
        get: () => e.modelValue,
        set: (l) => t("update:modelValue", l)
      }),
      formatter: (l) => {
        if (e.type !== "number")
          return l;
        let a = l;
        a.substr(0, 1) === "0" && a.length === 2 && !a.includes(".") && (a = a.substr(1, a.length));
        const c = new RegExp(`[^0-9]{0,1}(\\d*(?:\\.\\d{0,${e.point}})?).*$`, "g");
        return a = a.replace(c, "$1") ?? "", Number(a) < e.min ? e.min : Number(a) > e.max ? e.max : a;
      }
    };
  }
}), lr = { class: "flex-center k-input" };
function ir(e, t, n, o, l, a) {
  const c = St("van-field");
  return F(), re("div", lr, [
    me("span", null, pe(e.prefix), 1),
    f(c, fe({
      modelValue: e.inputValue,
      "onUpdate:modelValue": t[0] || (t[0] = (r) => e.inputValue = r),
      type: e.type
    }, e.$attrs, { formatter: e.formatter }), null, 16, ["modelValue", "type", "formatter"]),
    me("span", null, pe(e.suffix), 1)
  ]);
}
const bt = /* @__PURE__ */ dt(ar, [["render", ir]]);
bt.install = function(e) {
  e.component(bt.name, bt);
};
const rr = Q({
  name: "KvTable",
  props: {
    align: {
      // 对齐方式
      type: String,
      default: "left",
      validate: (e) => ["left", "center", "right"].includes(e)
    },
    columns: { type: Array, default: () => [] },
    data: { type: Array, default: () => [] },
    headerStyle: { type: Object, default: () => ({}) },
    rowStyle: { type: Object, default: () => ({}) },
    border: { type: Boolean, default: !1 },
    showOverflowTooltip: { type: Boolean, default: !1 },
    emptyText: { type: String, default: "暂无数据" }
  },
  emits: ["row-click"],
  setup(e, { emit: t }) {
    return { columnStyle: O(() => function(l) {
      const a = Number(l.width) ? `${l.width}px` : l.width;
      return [
        `text-align:${e.align}`,
        e.border ? "border-bottom: 1px solid #ebedf0;padding-bottom: 6px" : "",
        e.rowStyle,
        l.width ? `width: ${a}` : "flex: 1"
      ];
    }), clickRow: (l, a) => t("row-click", l, a) };
  }
}), cr = { class: "k-table bg-white mt10 p20" }, sr = { class: "table-content" }, ur = { class: "table-body" }, dr = { key: 1 }, fr = {
  key: 0,
  class: "flex-center p20"
}, vr = { class: "color-99" };
function gr(e, t, n, o, l, a) {
  return F(), re("div", cr, [
    me("div", sr, [
      me("div", {
        class: "table-header flex",
        style: Nt(e.headerStyle)
      }, [
        (F(!0), re(Ne, null, $e(e.columns, (c) => (F(), re("div", {
          key: c.prop,
          class: "table-column",
          style: Nt(e.columnStyle(c))
        }, pe(c.label), 5))), 128))
      ], 4),
      me("div", ur, [
        (F(!0), re(Ne, null, $e(e.data, (c, r) => (F(), re("div", {
          key: r,
          class: "flex table-column column-item flex-align-center"
        }, [
          (F(!0), re(Ne, null, $e(e.columns, (i) => (F(), re("div", {
            key: i.prop,
            style: Nt(e.columnStyle(i)),
            class: cn({ "text-overflow": e.showOverflowTooltip })
          }, [
            e.$slots[(i == null ? void 0 : i.custom) ?? (i == null ? void 0 : i.prop)] ? ke(e.$slots, i.custom ?? i.prop, {
              key: 0,
              row: c,
              index: r
            }, void 0, !0) : (F(), re("span", dr, pe(c[i.prop]), 1))
          ], 6))), 128))
        ]))), 128)),
        e.data.length ? et("", !0) : (F(), re("div", fr, [
          ke(e.$slots, "empty", {}, () => [
            me("span", vr, pe(e.emptyText), 1)
          ], !0)
        ]))
      ])
    ])
  ]);
}
const kt = /* @__PURE__ */ dt(rr, [["render", gr], ["__scopeId", "data-v-a71233b4"]]);
kt.install = function(e) {
  e.component(kt.name, kt);
};
const Pn = Q({
  name: "KvTree",
  components: { VanIcon: Te },
  props: {
    modelValue: { type: Array, default: () => [] },
    depth: { type: Number, default: 0 },
    props: { type: Object, default: () => ({}) },
    theme: { type: String, default: "#f4364c" },
    select: { type: [String, Number], default: "" }
  },
  emits: ["update:modelValue", "update:select", "click", "change"],
  setup(e, { emit: t }) {
    const n = O({
      get: () => e.modelValue,
      set: (s) => t("update:modelValue", s)
    }), o = O(() => ({
      name: "name",
      id: "id",
      child: "child",
      disabled: "disabled",
      ...e.props
    })), l = (s) => (s.forEach((g) => {
      g.showChildren = !1, g[o.value.child] && l(g[o.value.child]);
    }), s), a = E(e.select), c = (s) => {
      s[o.value.disabled] || (s[o.value.child] && s.showChildren || (n.value = l(n.value)), s.showChildren = !s.showChildren, a.value = s[o.value.id], r(s));
    }, r = (s) => {
      t("update:select", s[o.value.id]), t("change", s), t("click", s);
    }, i = (s, g) => s.disabled ? "not-allowed" : s[o.value.id] === a.value ? {
      0: "first-depth c-white",
      1: "second-depth c-white",
      2: "third-depth c-white"
    }[g] : "", u = O(() => e.theme);
    return {
      list: n,
      clickItem: c,
      clickChild: r,
      getClassName: i,
      themecolor: u,
      deaultProps: o
    };
  }
}), Fn = () => {
  to((e) => ({
    "80a71366": e.themecolor
  }));
}, Vn = Pn.setup;
Pn.setup = Vn ? (e, t) => (Fn(), Vn(e, t)) : Fn;
const mr = { class: "k-tree" }, Ar = ["onClick"], hr = { class: "flex-center flex1 tree-item-content" }, yr = { class: "mr10" };
function br(e, t, n, o, l, a) {
  const c = St("van-icon"), r = St("kv-tree");
  return F(), re("div", mr, [
    (F(!0), re(Ne, null, $e(e.list, (i) => (F(), re(Ne, {
      key: i[e.deaultProps.id]
    }, [
      me("div", {
        class: cn(["tree-item p10 flex-center", e.getClassName(i, e.depth)]),
        onClick: Wo((u) => e.clickItem(i, e.depth), ["stop"])
      }, [
        me("div", hr, [
          me("span", yr, pe(i[e.deaultProps.name]), 1),
          i[e.deaultProps.child] ? (F(), Se(c, {
            key: 0,
            name: i.showChildren ? "arrow-up" : "arrow-down"
          }, null, 8, ["name"])) : et("", !0)
        ])
      ], 10, Ar),
      i.showChildren && i[e.deaultProps.child] ? (F(), Se(r, {
        key: 0,
        modelValue: i[e.deaultProps.child],
        "onUpdate:modelValue": (u) => i[e.deaultProps.child] = u,
        props: e.deaultProps,
        theme: e.theme,
        depth: e.depth + 1,
        onClick: e.clickChild
      }, null, 8, ["modelValue", "onUpdate:modelValue", "props", "theme", "depth", "onClick"])) : et("", !0)
    ], 64))), 128))
  ]);
}
const Pt = /* @__PURE__ */ dt(Pn, [["render", br], ["__scopeId", "data-v-31963786"]]);
Pt.install = function(e) {
  e.component(Pt.name, Pt);
};
const kr = "data:image/png;base64,", Pr = "data:image/png;base64,", Or = "data:image/png;base64,";
const Er = Q({
  name: "KvStatus",
  props: {
    type: { type: String, default: "data" }
  },
  setup(e) {
    return { imgUrl: O(() => new URL((/* @__PURE__ */ Object.assign({ "./images/data.png": kr, "./images/fail.png": Pr, "./images/loading.png": Or }))[`./images/${e.type}.png`], self.location).href) };
  }
}), Sr = { class: "kv-status flex-column" }, Ir = { class: "error-type flex-center" }, xr = { class: "status-svg" }, Tr = ["src"], Dr = { class: "mt20 flex-center font-size16 status-text" };
function zr(e, t, n, o, l, a) {
  return F(), re("div", Sr, [
    me("div", Ir, [
      ke(e.$slots, "image", {}, () => [
        me("div", xr, [
          me("img", {
            src: e.imgUrl,
            class: "status-image"
          }, null, 8, Tr)
        ])
      ], !0)
    ]),
    me("div", Dr, [
      ke(e.$slots, "default", {}, void 0, !0)
    ])
  ]);
}
const Ue = /* @__PURE__ */ dt(Er, [["render", zr], ["__scopeId", "data-v-4262cba6"]]);
Ue.install = function(e) {
  e.component(Ue.name, Ue);
};
const wr = { class: "kv-list flex-column flex1" }, Ot = /* @__PURE__ */ Object.assign({
  name: "KvList"
}, {
  __name: "main",
  props: {
    modelValue: { type: Number, default: 1 },
    disabled: { type: Boolean, default: !1 },
    // 是否禁用下拉刷新
    finishedText: { type: String, default: "没有更多了" },
    // 加载完成的文字
    loadRequest: { type: Function, default: () => {
    } },
    // load回调
    responseNames: { type: Object, default: () => ({}) },
    // response参数说明
    refreshChange: { type: Function, default: () => {
    } },
    // 刷新的回调
    keyName: { type: String, default: "" }
    // rowKey
  },
  setup(e, { expose: t }) {
    const n = e, o = E(!1), l = E(!1), a = E([]), c = E(1), r = E(!1), i = E(!1), u = E(!1), s = (y, S = {}) => {
      const I = { ...{ pageNo: "pageNo", totalPage: "totalPage", records: "records" }, ...n.responseNames };
      return S[I[y]];
    }, g = async () => {
      u.value && await n.refreshChange();
      const y = await n.loadRequest(c.value);
      u.value = !1, o.value = !1;
      const S = s("pageNo", y) ?? 1, m = s("totalPage", y) ?? 1, I = s("records", y) ?? [];
      a.value = c.value === 1 ? I : [...a.value, ...I], i.value = !a.value.length, S >= m && (l.value = !0), c.value++, r.value = !1;
    }, d = () => {
      c.value = 1, a.value = [], r.value = !0, i.value = !1, l.value = !1, o.value = !0, g();
    }, A = async () => {
      await n.refreshChange(), d();
    };
    return t({
      reset: d
    }), (y, S) => (F(), re("div", wr, [
      f(oe(tr), {
        modelValue: u.value,
        "onUpdate:modelValue": S[1] || (S[1] = (m) => u.value = m),
        disabled: e.disabled,
        onRefresh: A,
        class: "kv-pull flex1"
      }, {
        default: Pe(() => [
          o.value && (r.value || c.value === 1) ? (F(), Se(oe(Ue), {
            key: 0,
            type: "loading"
          }, {
            image: Pe(() => [
              ke(y.$slots, "loading")
            ]),
            _: 3
          })) : i.value ? (F(), Se(oe(Ue), { key: 1 }, {
            image: Pe(() => [
              ke(y.$slots, "empty")
            ]),
            _: 3
          })) : (F(), Se(oe(Vi), fe({
            key: 2,
            loading: o.value,
            "onUpdate:loading": S[0] || (S[0] = (m) => o.value = m),
            finished: l.value,
            "finished-text": e.finishedText,
            onLoad: g
          }, y.$attrs), {
            default: Pe(() => [
              ke(y.$slots, "default", { list: a.value }),
              y.$slots.row ? ke(y.$slots, "row", { key: 0 }, () => [
                (F(!0), re(Ne, null, $e(a.value, (m, I) => (F(), re(Ne, {
                  key: e.keyName ? m[e.keyName] : I
                }, [
                  rn(pe(I) + " ", 1),
                  ke(y.$slots, "default", {
                    row: m,
                    index: I
                  })
                ], 64))), 128))
              ]) : et("", !0)
            ]),
            _: 3
          }, 16, ["loading", "finished", "finished-text"]))
        ]),
        _: 3
      }, 8, ["modelValue", "disabled"])
    ]));
  }
});
Ot.install = function(e) {
  e.component(Ot.name, Ot);
};
function en(e) {
  return Ko() ? ($o(e), !0) : !1;
}
function tn(e) {
  return typeof e == "function" ? e() : oe(e);
}
const nn = typeof window < "u", Cr = /^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/, jr = /\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a{1,2}|A{1,2}|m{1,2}|s{1,2}|Z{1,2}|SSS/g;
function Hr(e, t, n, o) {
  let l = e < 12 ? "AM" : "PM";
  return o && (l = l.split("").reduce((a, c) => a += `${c}.`, "")), n ? l.toLowerCase() : l;
}
function Nr(e, t, n = {}) {
  var o;
  const l = e.getFullYear(), a = e.getMonth(), c = e.getDate(), r = e.getHours(), i = e.getMinutes(), u = e.getSeconds(), s = e.getMilliseconds(), g = e.getDay(), d = (o = n.customMeridiem) != null ? o : Hr, A = {
    YY: () => String(l).slice(-2),
    YYYY: () => l,
    M: () => a + 1,
    MM: () => `${a + 1}`.padStart(2, "0"),
    MMM: () => e.toLocaleDateString(n.locales, { month: "short" }),
    MMMM: () => e.toLocaleDateString(n.locales, { month: "long" }),
    D: () => String(c),
    DD: () => `${c}`.padStart(2, "0"),
    H: () => String(r),
    HH: () => `${r}`.padStart(2, "0"),
    h: () => `${r % 12 || 12}`.padStart(1, "0"),
    hh: () => `${r % 12 || 12}`.padStart(2, "0"),
    m: () => String(i),
    mm: () => `${i}`.padStart(2, "0"),
    s: () => String(u),
    ss: () => `${u}`.padStart(2, "0"),
    SSS: () => `${s}`.padStart(3, "0"),
    d: () => g,
    dd: () => e.toLocaleDateString(n.locales, { weekday: "narrow" }),
    ddd: () => e.toLocaleDateString(n.locales, { weekday: "short" }),
    dddd: () => e.toLocaleDateString(n.locales, { weekday: "long" }),
    A: () => d(r, i),
    AA: () => d(r, i, !1, !0),
    a: () => d(r, i, !0),
    aa: () => d(r, i, !0, !0)
  };
  return t.replace(jr, (y, S) => {
    var m;
    return S || ((m = A[y]) == null ? void 0 : m.call(A)) || y;
  });
}
function pr(e) {
  if (e === null)
    return /* @__PURE__ */ new Date(NaN);
  if (e === void 0)
    return /* @__PURE__ */ new Date();
  if (e instanceof Date)
    return new Date(e);
  if (typeof e == "string" && !/Z$/i.test(e)) {
    const t = e.match(Cr);
    if (t) {
      const n = t[2] - 1 || 0, o = (t[7] || "0").substring(0, 3);
      return new Date(t[1], n, t[3] || 1, t[4] || 0, t[5] || 0, t[6] || 0, o);
    }
  }
  return new Date(e);
}
function Ut(e, t = "HH:mm:ss", n = {}) {
  return O(() => Nr(pr(tn(e)), tn(t), n));
}
function Br(e, t = 1e3, n = {}) {
  const {
    immediate: o = !0,
    immediateCallback: l = !1
  } = n;
  let a = null;
  const c = E(!1);
  function r() {
    a && (clearInterval(a), a = null);
  }
  function i() {
    c.value = !1, r();
  }
  function u() {
    const s = tn(t);
    s <= 0 || (c.value = !0, l && e(), r(), a = setInterval(e, s));
  }
  if (o && nn && u(), $n(t) || typeof t == "function") {
    const s = Z(t, () => {
      c.value && nn && u();
    });
    en(s);
  }
  return en(i), {
    isActive: c,
    pause: i,
    resume: u
  };
}
const Rr = nn ? window : void 0;
function Jr(e, t = {}) {
  const {
    immediate: n = !0,
    window: o = Rr
  } = t, l = E(!1);
  let a = 0, c = null;
  function r(s) {
    if (!l.value || !o)
      return;
    const g = s - a;
    e({ delta: g, timestamp: s }), a = s, c = o.requestAnimationFrame(r);
  }
  function i() {
    !l.value && o && (l.value = !0, c = o.requestAnimationFrame(r));
  }
  function u() {
    l.value = !1, c != null && o && (o.cancelAnimationFrame(c), c = null);
  }
  return n && i(), en(u), {
    isActive: _o(l),
    pause: u,
    resume: i
  };
}
var Mr = Object.defineProperty, Wn = Object.getOwnPropertySymbols, Zr = Object.prototype.hasOwnProperty, Yr = Object.prototype.propertyIsEnumerable, Kn = (e, t, n) => t in e ? Mr(e, t, { enumerable: !0, configurable: !0, writable: !0, value: n }) : e[t] = n, Xr = (e, t) => {
  for (var n in t || (t = {}))
    Zr.call(t, n) && Kn(e, n, t[n]);
  if (Wn)
    for (var n of Wn(t))
      Yr.call(t, n) && Kn(e, n, t[n]);
  return e;
};
function Ur(e = {}) {
  const {
    controls: t = !1,
    interval: n = "requestAnimationFrame"
  } = e, o = E(/* @__PURE__ */ new Date()), l = () => o.value = /* @__PURE__ */ new Date(), a = n === "requestAnimationFrame" ? Jr(l, { immediate: !0 }) : Br(l, n, { immediate: !0 });
  return t ? Xr({
    now: o
  }, a) : o;
}
function Uo(e, t) {
  var d;
  const n = (d = Ut(Ur(), "YYYY-MM-DD")) == null ? void 0 : d.value, o = E(`${n} 00:00:00`), l = E(`${n} 23:59:59`), a = new Date((/* @__PURE__ */ new Date()).getTime() + 24 * 60 * 60 * 1e3), c = (A) => A && typeof A == "string" ? new Date(A) : A, r = (/* @__PURE__ */ new Date()).getFullYear(), i = new Date(r - 3, 1, 1), u = O(() => {
    const A = e.type ?? "range";
    if (A === "range") {
      const { startTime: y, endTime: S } = t.value || {};
      return [c(y) ?? /* @__PURE__ */ new Date(), c(S) ?? /* @__PURE__ */ new Date()];
    }
    return A === "multiple" ? null : c(t.value) ?? /* @__PURE__ */ new Date();
  });
  return {
    minDate: i,
    maxDate: a,
    initalDate: u,
    startData: o,
    endDate: l,
    useDateFormat: Ut,
    formarData: (A, y = "YYYY-MM-DD HH:mm:ss") => {
      var S;
      return (S = Ut(A, y)) == null ? void 0 : S.value;
    },
    formatter: (A) => ((/* @__PURE__ */ new Date()).getTime() === new Date(A.date).getTime() && (A.type = "disabled"), A),
    setStringToDate: c
  };
}
const Gr = { class: "edit-content" }, Qr = { class: "left-icon" }, Lr = ["value"], Gt = {
  __name: "picker-edit",
  props: {
    modelValue: { type: [String, Date], default: "" },
    showFormat: { type: String, default: "YYYY-MM-DD" }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: t }) {
    const n = e, { formarData: o, setStringToDate: l } = Uo(), a = sn(), c = O({
      get: () => n.modelValue ? o(l(n.modelValue), a.showFormat ?? n.showFormat) : "",
      set: (i) => t("update:modelValue", i)
    }), r = O(() => ({
      "picker-border": ["round-border", "border"].includes(a.shape),
      "picker-border-round": ["round-border", "round"].includes(a.shape)
    }));
    return (i, u) => (F(), re("div", fe({
      class: ["picker-edit flex-align-center flex", [r.value]]
    }, i.$attrs), [
      me("div", Gr, [
        me("div", Qr, [
          (i.$attrs.showIcon, F(), Se(oe(Te), {
            key: 0,
            name: "underway-o"
          }))
        ])
      ]),
      me("input", {
        class: "edit-control",
        value: c.value,
        disabled: "",
        placeholder: "开始时间"
      }, null, 8, Lr)
    ], 16));
  }
};
const qr = { class: "date-shortcuts mr10 flex-shrink" }, Fr = {
  __name: "shortcuts",
  props: {
    shortcutsValue: { type: Number, default: -1 }
  },
  emits: ["confirm", "change"],
  setup(e, { emit: t }) {
    const n = e, o = E(!1), l = [
      { text: "昨天", value: 1 },
      { text: "今天", value: 0 },
      { text: "近7天", value: 7 },
      { text: "近一个月", value: 30 },
      { text: "近3个月", value: 90 }
    ], a = sn(), c = O(() => ["round", "round-border"].includes(a.shape)), r = O(() => ["round", "default"].includes(a.shape)), i = E("选择范围");
    Ee(() => {
      const s = l.find((g) => g.value === n.shortcutsValue);
      s && (i.value = s.text, u({ selectedValues: [s.value], selectedOptions: [s] }));
    });
    const u = ({ selectedValues: s, selectedOptions: g }) => {
      const [d] = g;
      i.value = d.text, o.value = !1;
      const [A] = s, S = (/* @__PURE__ */ new Date()).getTime() - 3600 * 1e3 * 24 * A;
      t("confirm", { value: S, select: A });
    };
    return (s, g) => (F(), re("div", qr, [
      f(oe(An), {
        round: c.value,
        onClick: g[0] || (g[0] = (d) => o.value = !0),
        class: cn({ "bg-button": r.value })
      }, {
        default: Pe(() => [
          rn(pe(i.value), 1)
        ]),
        _: 1
      }, 8, ["round", "class"]),
      f(oe(hn), {
        show: o.value,
        "onUpdate:show": g[2] || (g[2] = (d) => o.value = d),
        position: "bottom",
        "safe-area-inset-bottom": ""
      }, {
        default: Pe(() => [
          f(oe(Kl), {
            columns: l,
            onConfirm: u,
            onCancel: g[1] || (g[1] = (d) => o.value = !1)
          })
        ]),
        _: 1
      }, 8, ["show"])
    ]));
  }
}, Vr = { class: "k-date-picker flex-align-center" }, Et = /* @__PURE__ */ Object.assign({
  name: "KvDatePicker"
}, {
  __name: "main",
  props: {
    modelValue: { type: [Object, String], default: () => ({}) },
    showShortcuts: { type: Boolean, default: !1 },
    shortcutsValue: { type: Number, default: -1 }
  },
  emits: ["update:modelValue", "confirm"],
  setup(e, { emit: t }) {
    const n = e, o = O({
      get: () => n.modelValue,
      set: (m) => t("update:modelValue", m)
    }), l = E(!1), a = sn(), c = O(() => a.type ?? "range"), {
      minDate: r,
      maxDate: i,
      initalDate: u,
      formarData: s,
      formatter: g,
      setStringToDate: d
    } = Uo(a, o), A = E(u.value), y = (m) => {
      const I = c.value, x = a.valueFormat ?? "YYYY-MM-DD HH:mm:ss";
      if (I === "range") {
        const M = s(m[0], x), w = x === "YYYY-MM-DD HH:mm:ss" ? `${s(m[1], "YYYY-MM-DD")} 23:59:59` : `${s(m[1], x)}`;
        o.value = { startTime: M, endTime: w }, A.value = [new Date(M), new Date(w)];
      } else if (I === "multiple") {
        const M = m.map((w) => `${s(w, x)}`);
        o.value = M, A.value = M;
      } else
        o.value = s(m, x), A.value = d(o.value);
      l.value = !1, te(() => t("confirm", o.value));
    }, S = ({ value: m, select: I }) => {
      const x = `${s(m, "YYYY-MM-DD")} 00:00:00`, M = I === 1 ? x : `${s(/* @__PURE__ */ new Date(), "YYYY-MM-DD")} 23:59:00`;
      o.value = { startTime: x, endTime: M }, y([x, M]);
    };
    return (m, I) => (F(), re("div", Vr, [
      e.showShortcuts && c.value === "range" ? (F(), Se(Fr, fe({ key: 0 }, m.$attrs, {
        "shortcuts-value": e.shortcutsValue,
        onConfirm: S
      }), null, 16, ["shortcuts-value"])) : et("", !0),
      ["single"].includes(m.$attrs.type) ? (F(), Se(Gt, fe({ key: 1 }, m.$attrs, {
        modelValue: o.value,
        "onUpdate:modelValue": I[0] || (I[0] = (x) => o.value = x),
        onClick: I[1] || (I[1] = (x) => l.value = !0)
      }), null, 16, ["modelValue"])) : (F(), Se(oe(nr), {
        key: 2,
        justify: "space-between"
      }, {
        default: Pe(() => [
          f(oe(Xt), {
            span: "11",
            class: "calendar-col",
            onClick: I[3] || (I[3] = (x) => l.value = !0)
          }, {
            default: Pe(() => [
              f(Gt, fe(m.$attrs, {
                modelValue: o.value.startTime,
                "onUpdate:modelValue": I[2] || (I[2] = (x) => o.value.startTime = x)
              }), null, 16, ["modelValue"])
            ]),
            _: 1
          }),
          f(oe(Xt), {
            span: "1",
            class: "calendar-line"
          }),
          f(oe(Xt), {
            span: "11",
            class: "calendar-col",
            onClick: I[5] || (I[5] = (x) => l.value = !0)
          }, {
            default: Pe(() => [
              f(Gt, fe({ placeholder: "结束日期" }, m.$attrs, {
                modelValue: o.value.endTime,
                "onUpdate:modelValue": I[4] || (I[4] = (x) => o.value.endTime = x)
              }), null, 16, ["modelValue"])
            ]),
            _: 1
          })
        ]),
        _: 1
      })),
      f(oe(Ri), fe({
        teleport: "body",
        class: "overflow",
        show: l.value,
        "onUpdate:show": I[6] || (I[6] = (x) => l.value = x),
        type: "range",
        onConfirm: y,
        "allow-same-day": "",
        "max-date": oe(i),
        "min-date": oe(r),
        formatter: oe(g),
        "safe-area-inset-bottom": "",
        "default-date": A.value
      }, m.$attrs), null, 16, ["show", "max-date", "min-date", "formatter", "default-date"])
    ]));
  }
});
Et.install = function(e) {
  e.component(Et.name, Et);
};
const Qt = {
  KvButton: yt,
  KvInput: bt,
  KvTable: kt,
  KvTree: Pt,
  KvList: Ot,
  KvStatus: Ue,
  KvDatePicker: Et,
  install: () => {
  }
};
function Wr(e, t, n = 0) {
  return e.substr(n, t.length) === t;
}
Qt.install = function(e) {
  Object.keys(Qt).forEach((t) => {
    if (Wr(t, "K")) {
      const n = Qt[t];
      e.component(n.name, n);
    }
  });
};
export {
  Qt as KVant,
  yt as KvButton,
  Et as KvDatePicker,
  bt as KvInput,
  Ot as KvList,
  Ue as KvStatus,
  kt as KvTable,
  Pt as KvTree
};
